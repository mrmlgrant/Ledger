...
function cgtPlanner(assetId){
  ...
  const y=FY();const s=fySummaryNumbers(y,pid);
  let mRate=0;
  for(const br of y.rates.brackets){if(s.taxable>=(br.upTo||Infinity))mRate=br.rate;else{mRate=br.rate;break;}}
  const _cgtAid=assetId;
  modal('What if I sell? — '+esc(a.name),
    '<div class="hint" style="margin-bottom:14px">Estimate your capital gains tax. Uses average cost method and your current FY marginal rate.</div>'
    +'<div class="fldrow">'
    +'<div><label class="fld">Units to sell</label><input id="cgt_qty" class="input money" value="'+heldQty+'" oninput="cgtPlannerCalc(this.closest(\'.modal\').dataset.aid)"></div>'
    +'<div><label class="fld">Sale price per unit ($)</label><input id="cgt_price" class="input money" value="'+(lastPrice||'')+'" oninput="cgtPlannerCalc(this.closest(\'.modal\').dataset.aid)"></div></div>'
    +'<div id="cgt_result" style="margin-top:16px"></div>'
    +'<div class="note mt">Earliest buy in pool: <b>'+(earliestBuy?fmtDate(earliestBuy):'unknown')+'</b> — '+(discountEligible?'<span style="color:var(--euc)">eligible for 50% CGT discount (held >12 months)</span>':'<span style="color:var(--gold-badge)">not yet held 12 months — no CGT discount</span>.')+'. Marginal rate: <b>'+mRate+'%</b> on taxable income of '+fmt$0(s.taxable)+'. Estimate only — not tax advice.</div>',
    '<button class="btn" data-close>Close</button>');
...
function cgtPlannerCalc(assetId){
  ...
  const qty=Math.min(num($('#cgt_qty')?.value),heldQty);
  ...
  let mRate=0;
  for(const br of y.rates.brackets){if(s.taxable>=(br.upTo||Infinity))mRate=br.rate;else{mRate=br.rate;break;}}
  const discountedGain=discountEligible?rawGain*0.5:rawGain;
  const addedTaxable=Math.max(0,discountedGain);
  const estimatedTax=addedTaxable*(mRate/100);
  const isLoss=rawGain<0;
  const row=(label,val,style)=>'<div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px dashed var(--line);font-size:.88rem;'+(style||'')+'"><span style="color:var(--muted)">'+label+'</span><span style="font-variant-numeric:tabular-nums;font-weight:var(--fw-semibold)">'+val+'</span></div>';
  el.innerHTML='<div class="card"><div class="cbody">'
    +row('Sale proceeds',fmt$0(proceeds),'')
    +row('Cost basis (avg cost x units)',fmt$0(costForUnits),'')
    +row(isLoss?'Capital loss':'Capital gain',fmt$0(Math.abs(rawGain)),isLoss?'color:var(--euc)':'color:var(--red)')
    +(discountEligible?row('After 50% CGT discount (held >12 months)',fmt$0(discountedGain),'color:var(--euc)'):'')
    +(isLoss
      ?'<div style="padding:10px 0;font-size:.88rem;color:var(--euc)">Capital loss of '+fmt$0(Math.abs(rawGain))+' — offsets current or future capital gains. No tax payable on this disposal.</div>'
      :row('Estimated additional tax',fmt$0(estimatedTax),'font-size:.95rem')
       +'<div style="font-size:.75rem;color:var(--muted);margin-top:6px">At '+mRate+'% marginal rate on '+fmt$0(addedTaxable)+' taxable gain'+(discountEligible?' (after 50% discount)':'')+'</div>')
    +'</div></div>';
}
