...
PAGES.expenses=m=>{
  ...
  // Subscriptions tracker — show only if any Subscriptions category expenses exist
  const subs=(B.expenses||[]).filter(e=>e.category==='Subscriptions');
  if(subs.length){
    const recurSubs=subs.filter(e=>e.recurrence!=='once');
    const onceSubs=subs.filter(e=>e.recurrence==='once');
    const subMonthly=recurSubs.reduce((s,e)=>s+yearlyFromRec(num(e.amount),e.recurrence)/12,0);
    const subRows=recurSubs.map(e=>{
      const mo=yearlyFromRec(num(e.amount),e.recurrence)/12;
      const pct=subMonthly>0?Math.round(mo/subMonthly*100):0;
      const idx=(B.expenses||[]).indexOf(e);
      return '<div style="display:flex;align-items:center;justify-content:space-between;padding:7px 0;border-bottom:1px dashed var(--line);gap:8px">'
        +'<div style="flex:1;min-width:0">'
        +'<div style="font-size:.88rem;font-weight:var(--fw-semibold)">'+esc(e.name)+'</div>'
        +'<div style="font-size:.73rem;color:var(--muted)">'+(RECURRENCE_OPTS.find(([v])=>v===e.recurrence)?.[1]||e.recurrence)+' · '+fmt$(e.amount)+'</div>'
        +'<div style="height:3px;background:var(--line);border-radius:2px;margin-top:5px;width:'+pct+'%;max-width:100%;background:var(--blue)"></div>'
        +'</div>'
        +'<div style="font-size:.9rem;font-weight:var(--fw-semibold);font-variant-numeric:tabular-nums;white-space:nowrap">'+fmt$(mo)+'/mo</div>'
        +'<button class="btn ghost small" onclick="expAdd('+idx+')">Edit</button>'
        +'</div>';
    }).join('');
    const onceNote=onceSubs.length
      ?'<div class="note" style="margin-top:10px">📋 '+onceSubs.length+' one-off item'+(onceSubs.length>1?'s':'')+' in Subscriptions ('+onceSubs.map(e=>esc(e.name)).join(', ')+') — these are one-time charges. Edit to set a recurrence if they repeat.</div>':'';
    m.insertAdjacentHTML('beforeend','<div class="card" style="margin-top:18px">'
      +'<div class="chead"><h2>📱 Subscriptions</h2><span style="font-size:.84rem;color:var(--muted)">'+fmt$(subMonthly)+'/mo committed</span></div>'
      +'<div class="cbody">'+(subRows||'<div class="muted">No recurring subscriptions.</div>')
      +(recurSubs.length?'<div style="display:flex;justify-content:space-between;padding:10px 0 2px;font-weight:var(--fw-bold);border-top:3px double var(--line2);margin-top:4px"><span>Monthly committed</span><span style="font-variant-numeric:tabular-nums">'+fmt$(subMonthly)+'/mo · '+fmt$0(subMonthly*12)+'/yr</span></div>':'')
      +onceNote
      +'</div></div>');
  }
};
