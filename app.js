...
function enterOfflineMode(){
  ...
  if(hasData){
    // Local IndexedDB data exists — go straight in
    showApp();
    return;
  }
  // No local data — ask whether to import a backup or start fresh
  modal('Start in offline mode',
    `<p style="margin-bottom:16px;color:var(--muted);font-size:.9rem;line-height:1.6">No local data found on this device. How would you like to start?</p>
    <div style="display:flex;flex-direction:column;gap:10px">
      <button class="btn primary" style="justify-content:center" onclick="closeModal();document.getElementById('importFile').click()">
        📂 Import a backup file
      </button>
      <button class="btn" style="justify-content:center" onclick="closeModal();showApp()">
        ✦ Start fresh
      </button>
    </div>`,
    `<button class="btn ghost" data-close onclick="OFFLINE_MODE=false">Cancel</button>`
  );
}
...
async function authSignOut(){
  ...
  // Sync to Drive first if connected so the user's data is current before
  // they leave — they might be switching to offline mode or another device.
  if(GD.isConnected()){
    try{
      await pushToDrive();
    }catch(e){
      // Non-fatal — proceed with sign-out even if sync fails
    }
  }
  ...
  OFFLINE_MODE=false;
  save();
  // Hide the offline banner if it was showing
  const banner=document.getElementById('offlineBanner');
  if(banner)banner.style.display='none';
