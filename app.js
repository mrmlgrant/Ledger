...
async function rcptView(id,name){
  ...
  try{
    ...
    // Encode dataUrl into the onclick as a JS string literal so the click handlers
    // can use it synchronously — no await, no popup blocking, no re-fetch needed.
    const dataUrlJs=JSON.stringify(dataUrl);
    const filenameJs=JSON.stringify(rec.name||'receipt');
    modal(name||'Receipt',body,
      `<button class="btn" onclick="(()=>{const a=document.createElement('a');a.href=${dataUrlJs};a.download=${filenameJs};a.click();})()">Download</button>
       <button class="btn primary" onclick="(()=>{const w=window.open('','_blank');if(w){w.document.write('<html><body style=margin:0><embed src=${dataUrlJs.replace(/'/g,"\\'")} width=100% height=100% type=${JSON.stringify(mime)}></body></html>');w.document.close();}else{const a=document.createElement('a');a.href=${dataUrlJs};a.target='_blank';a.click();}})()">Open in new tab</button>
