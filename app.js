...
function preTaxDedYearlyFor(r,y){
  ...
  if(r.recurrence==='once')return base; // one-off: already the exact amount
  // Determine effective start: explicit startDate > leaseStartDate > nothing
  const startDate=r.startDate||(r.type==='Novated lease'?r.leaseStartDate:'')||'';
  if(!startDate||!y)return base;
  const{start:fyStart,end:fyEnd}=fyRange(y);
  if(startDate>fyEnd)return 0; // hasn't started yet this FY
  if(startDate<=fyStart)return base; // fully active all FY
  // Prorate: active from startDate to end of FY
  const totalDays=(new Date(fyEnd+'T00:00:00Z')-new Date(fyStart+'T00:00:00Z'))/86400000+1;
  const activeDays=(new Date(fyEnd+'T00:00:00Z')-new Date(startDate+'T00:00:00Z'))/86400000+1;
