...
async function rcptView(id,name){
  ...
  // File (not Blob) — browsers use the File's name as the suggested save filename
  // in the PDF viewer download button and the image download link.
  const file=new File([rawBlob],filename,{type:mime});
  const url=URL.createObjectURL(file);
  const esc2=s=>String(s).replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;');
  const isImg=mime.startsWith('image/');
  if(isImg){
    // Images: write a dark-background viewer with a named download link
    w.document.open();
    w.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>${esc2(filename)}</title>
    <style>*{margin:0;padding:0;box-sizing:border-box}body{background:#111;display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh}
    img{max-width:100vw;max-height:calc(100vh - 48px);object-fit:contain}
    .bar{position:fixed;bottom:0;left:0;right:0;height:48px;background:rgba(0,0,0,.7);display:flex;align-items:center;justify-content:center;gap:16px}
    .bar a{color:#fff;font:14px system-ui;text-decoration:none;padding:6px 14px;border:1px solid rgba(255,255,255,.4);border-radius:6px}
    .bar a:hover{background:rgba(255,255,255,.15)}</style></head>
    <body><img src="${esc2(url)}" alt="${esc2(filename)}">
    <div class="bar"><a href="${esc2(url)}" download="${esc2(filename)}">⬇ Download ${esc2(filename)}</a></div>
    </body></html>`);
    w.document.close();
  }else{
    // PDFs and other files: navigate directly to the file URL.
    // The browser's native PDF viewer handles it fullscreen, and because
    // the URL was created from a File object (not a bare Blob), Chrome,
    // Safari, and Edge use the filename as the suggested save name.
    w.location.href=url;
  }
