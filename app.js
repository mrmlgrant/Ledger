...
function budTotals(y,b){
  ...
  const net=gross-tax, excess=net-expsTotal;
  return{pids,perP,gross,tax,exps:expsTotal,dedExpsTotal,net,excess};
}
function budRows(b,list,kind,withPerson,y){
  ...
  const rowHtml=(r,i)=>{
    ...
    return `<tr>
    <td><b>${esc(r.name)||'<span class="muted">Unnamed</span>'}</b>${kind==='incomes'&&r.includesSuper?' <span class="badge gold" style="font-size:.68rem">incl. super</span>':''}${dedAmt>0?` <span class="badge euc" style="font-size:.68rem">✓ ${fmt$0(dedAmt)}/yr deductible</span>`:''}
