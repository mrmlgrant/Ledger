...
let _rcptViewDataUrl=null,_rcptViewMime=null,_rcptViewFilename=null;
function rcptViewDownload(){
  if(!_rcptViewDataUrl)return;
  const a=document.createElement('a');
  a.href=_rcptViewDataUrl;
  a.download=_rcptViewFilename;
  a.click();
}
function rcptViewOpen(){
  if(!_rcptViewDataUrl)return;
  const w=window.open('','_blank');
  if(w){
    // Write the content into the new tab — avoids popup-blocker issues that
    // occur when window.open() is called after an await (async break).
    if(_rcptViewMime&&_rcptViewMime.includes('pdf')){
      w.document.write(`<!DOCTYPE html><html><head><style>*{margin:0;padding:0}html,body{height:100%;overflow:hidden}</style></head><body><embed src="${_rcptViewDataUrl}" type="application/pdf" width="100%" height="100%"></body></html>`);
      w.document.close();
    }else{
      w.document.write(`<!DOCTYPE html><html><head><style>*{margin:0;padding:0;background:#000}body{display:flex;align-items:center;justify-content:center;min-height:100vh}img{max-width:100vw;max-height:100vh;object-fit:contain}</style></head><body><img src="${_rcptViewDataUrl}"></body></html>`);
      w.document.close();
    }
  }else{
    // Popup was blocked — fall back to forcing a download
    rcptViewDownload();
    toast('Popup blocked — file downloaded instead. Allow popups for this site to open in a new tab.');
  }
}
async function rcptView(id,name){
  ...
  try{
    ...
    // Store in module-level vars so onclick handlers can reference them
    // synchronously — no async, no await, no popup blocking.
    _rcptViewDataUrl=dataUrl;
    _rcptViewMime=mime;
    _rcptViewFilename=rec.name||'receipt';
    modal(name||'Receipt',body,
      `<button class="btn" onclick="rcptViewDownload()">Download</button>
       <button class="btn primary" onclick="rcptViewOpen()">Open in new tab</button>
