...
function docsCollect(years,pids){
  ...
  years.forEach(y=>{
    ...
    pids.forEach(pid=>{
      ...
      // General expenses — only collect those that have a receipt attached
      // (attaching is optional for general expenses; we don't list them as
      // "missing" since most everyday expenses legitimately have no receipt)
      (b.expenses||[]).filter(e=>e.receiptId).forEach(e=>{
