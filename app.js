...
let OFFLINE_MODE=false; // transient — user entered the app without Google auth
...
function ratesFY2025(){return{
  ...
  superSGRate:11.5,
  // Private health insurance (PHI) rebate — means-tested, reduces with income.
  // Percentages are for individuals aged under 65 (65-69: add ~4.1pp, 70+: add ~8.2pp).
  // Verify with ATO each year — indexed annually.
  phi:{
    tiers:[
      {maxIncome:93000, rebate:24.608},  // Base tier — full rebate
      {maxIncome:108000,rebate:16.405},  // Tier 1
      {maxIncome:144000,rebate:8.202},   // Tier 2
      {maxIncome:null,  rebate:0}        // Tier 3 — no rebate
    ],
    ageAdj:{under65:0,age65to69:4.102,age70plus:8.204}
  }
};}
...
function newBucket(){return{
  ...
  summaryOpts:{medicare:true,mls:false,hasCover:true,hasPHI:false,phiPremium:0,phiAge:'under65'},
...
function newFY(startYear,rates){
  ...
  locked:false, archived:false, rates, extraHolidays:[], removedHolidays:[], webHolidays:null,
  budgets:[],
...
function ensureDbShape(db){
  ...
  Object.values(db.years).forEach(y=>{
    ...
    if(y.archived===undefined)y.archived=false;
  // Per-asset CGT 2027 valuation fields — added for Budget 2026-27 transition
  (db.assets||[]).forEach(a=>{
    if(a.cgt2027Value===undefined)a.cgt2027Value=null;   // recorded value/price at 1 Jul 2027
    if(a.cgt2027Confirmed===undefined)a.cgt2027Confirmed=false; // user confirmed the valuation
    if(a.depReportId===undefined)a.depReportId='';   // attached depreciation schedule PDF
    if(a.depReportName===undefined)a.depReportName='';
  });
...
function barChartSVG(data,opts){ /* data:[{label,value,value2?}] */
  ...
  data.forEach((d,i)=>{
    ...
    if(v1===0&&!v2)bars+=`<rect x="${x}" y="${H-padB-3}" width="${w}" height="3" rx="1" fill="${color1}" opacity=".18"/>`;
...
function lineChartSVG(pts,opts){ /* pts:[{label,value}] */
  ...
  const valLbls=pts.map((p,i)=>{if(i%step!==0&&i!==pts.length-1)return'';const cx=X(i).toFixed(1),cy=(Y(num(p.value))-10).toFixed(1);const anchor=i===0?'start':i===pts.length-1?'end':'middle';return`<text class="vlab" x="${cx}" y="${cy}" text-anchor="${anchor}">${cfmt(p.value)}</text>`;}).join('');
  ...
  const labels=pts.map((p,i)=>{
    ...
    if(manyPts)return`<text class="axis" x="${cx}" y="${H-padB+13}" text-anchor="end" transform="rotate(-40 ${cx} ${H-padB+13})">${esc(p.label)}</text>`;
    // Anchor edge labels inward so they don't overflow the viewBox — the
    // first/last points sit exactly at padL/W-padR, so a centered label
    // there would extend past the edge and get clipped.
    const anchor=i===0?'start':i===pts.length-1?'end':'middle';
    return`<text class="axis" x="${cx}" y="${H-padB+15}" text-anchor="${anchor}">${esc(p.label)}</text>`;
  }).join('');
...
function budMonthlyFlow(rows,y){
  const r=new Array(12).fill(0);
  const fyStart=y?fyRange(y).start:null;
  (rows||[]).forEach(row=>{
    const per=row.per||'m';
    const yearly=yrOf(row,y);
    if(yearly<=0)return;
    // FY-relative month index (0=first month of FY) this row first
    // becomes active in, if it has a start date within this FY.
    let startIdx=0;
    if(y&&row.startDate&&row.startDate>fyStart){
      const d=new Date(row.startDate+'T00:00:00Z');
      startIdx=budCalMonthToFYIdx(d.getUTCMonth()+1);
    }
    if(per==='y'){
      const idx=budCalMonthToFYIdx(row.schedDay);
      if(idx>=startIdx)r[idx]+=yearly;
    }else if(per==='b'){
      const m1=num(row.schedDay)||1,m2=((m1-1+6)%12)+1;
      const i1=budCalMonthToFYIdx(m1),i2=budCalMonthToFYIdx(m2);
      const half=yearly/2;
      if(i1>=startIdx)r[i1]+=half;
      if(i2>=startIdx)r[i2]+=half;
    }else{
      const activeMonths=12-startIdx;
      if(activeMonths>0){
        const perMonth=yearly/activeMonths;
        for(let i=startIdx;i<12;i++)r[i]+=perMonth;
      }
    }
  });
  return r;
  ...
  return `<div class="card" style="border-color:var(--gold);background:var(--gold-soft)">
    <div class="cbody" style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap">
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        <button class="btn primary small" onclick="settingsLock()">🔒 Lock ${esc(fyDisplay(y))}</button>
...
function dashToggle(k){
  ...
  if(DASH_OPEN){requestAnimationFrame(()=>{
    const el=document.querySelector('.dash-8grid .dex.open,.dash-stat-grid .dex.open');
...
PAGES.dashboard=m=>{
  ...
  // 'holding' items are auto-generated bookkeeping records (created by
  // assetFetchPrice purely to give the net worth chart dated history) and
  // are excluded here, exactly like the real Net Worth page does — their
  // value is added back below via the live assetCurrentValue() computation
  // instead of nw.entries, since a holding shows $0 here whenever no entry
  // has been recorded yet (e.g. price set without ever calling
  // assetFetchPrice), even though the asset itself has a perfectly good
  // current value.
  const nwItems=DB.nw?.items.filter(it=>it.pid===pid&&it.kind!=='holding')||[];
  const nwHoldingAssets=assetsForPerson([pid]).filter(a=>(a.kind==='shares'||a.kind==='managed_fund')&&assetVisibleInFY(a,y));
  ...
  },0)+nwHoldingAssets.reduce((sum,a)=>sum+(assetCurrentValue(a)||0),0);

  head(m,'Dashboard',`${fyDisplay(y)} · Overview`,
    `<button class="btn primary small" onclick="go('income')">+ Add income</button>`);
  ...
  const buildR1Dex=()=>{
    ...
    if(DASH_OPEN==='networth'){
      let rows=nwItems.map(it=>{
          const es=nwEntriesOf(it.id).filter(e=>e.date<=end);
          const v=es.length?es[es.length-1].value:null;
          if(v===null)return '';
          return dxRow(esc(it.name),fmt$(v),false,"go('networth')");
        }).filter(Boolean).join('');
      rows+=nwHoldingAssets.map(a=>{
          const v=assetCurrentValue(a);
          if(v===null)return '';
          return dxRow(esc(a.name)+(a.code?' ('+esc(a.code)+')':''),fmt$(v),false,"go('assets')");
        }).filter(Boolean).join('');
      if(!rows)rows='<div class="dex-row"><span class="muted">No net worth items yet — <a href="#" onclick="go(\'networth\')">add items</a>.</span></div>';
      else rows+=dxTot(`Net worth as at end of ${esc(fyDisplay(y))}`,fmt$(nwTotal));
  ...
  m.insertAdjacentHTML('beforeend',`<div class="dash-8grid" style="margin-bottom:6px">
    ...
    <div class="stat good" style="cursor:pointer" onclick="go('summary')" title="Go to FY summary">
      <div class="l">Taxable income</div><div class="v">${fmt$0(s.taxable)}</div>
      <div class="d">${fmt$0(s.assessable)} assessable − ${fmt$0(s.deductions)} deductions</div>
    </div>
    <div class="stat euc" style="cursor:pointer" onclick="go('other')" title="Go to deductions">
    ...
    <div class="stat gold" style="cursor:pointer" onclick="go('summary')" title="Go to FY summary">
      ...
      <div class="d">effective rate ${pct(s.effRate)}</div>
    </div>
    <div class="stat ${s.balance>=0?'good':'bad'}" style="cursor:pointer" onclick="go('summary')" title="Go to FY summary">
      <div class="l">${s.balance>=0?'Est. refund':'Est. owing'}</div>
      <div class="v">${fmt$0(Math.abs(s.balance))}</div>
      <div class="d">${s.balance>=0?'tax withheld exceeds liability':'more than withheld so far'}</div>
    </div>
  </div>
  ${buildR1Dex()}
  <div class="note" style="margin-bottom:18px">Tax figures are estimates from the ${esc(fyDisplay(y))} rate snapshot — not tax advice. <a href="#" onclick="go('summary');return false">See full FY summary →</a></div>`);
  ...
  m.insertAdjacentHTML('beforeend',`<div class="dash-row2" style="margin-bottom:18px">
    <div class="card"><div class="chead"><h2>Income vs expenses</h2>
      <span class="hint">Monthly — ${esc(fyDisplay(y))}</span></div>
      <div class="cbody">${barChartSVGClickable(cashData.map(d=>({...d,onClick:"go('income')",onClick2:"go('expenses')"})),{aria:'Monthly cashflow',label2:'Expenses'})}
        <div style="display:flex;gap:16px;font-size:var(--text-sm);margin-top:6px;color:var(--muted)">
          <a href="#" onclick="go('income');return false" style="display:flex;align-items:center;gap:5px;color:var(--muted);text-decoration:none" title="Go to Income page"><span style="display:inline-block;width:12px;height:12px;background:var(--euc);border-radius:3px;opacity:.85;vertical-align:-2px"></span>Income ↗</a>
          <a href="#" onclick="go('expenses');return false" style="display:flex;align-items:center;gap:5px;color:var(--muted);text-decoration:none" title="Go to Expenses page"><span style="display:inline-block;width:12px;height:12px;background:var(--gold);border-radius:3px;opacity:.85;vertical-align:-2px"></span>Expenses ↗</a>
        </div>
      </div>
    </div>
  ...
  // Tax estimate nudge — only for current unlocked FY with income and meaningful owing amount
  if(!y.locked&&s.assessable>0){
    const owing=-s.balance;
    if(owing>500){
      let mRate=0;
      for(const br of y.rates.brackets){if(s.taxable>=(br.upTo||Infinity))mRate=br.rate;else{mRate=br.rate;break;}}
      const suggestSuper=mRate>15;
      m.insertAdjacentHTML('beforeend',
        '<div class="card" style="border-color:var(--gold-badge);margin-bottom:18px">'
        +'<div class="chead" style="background:color-mix(in srgb,var(--gold-soft) 40%,var(--surface))">'
        +'<h2 style="color:var(--gold-badge)">⚠ Tax estimate: owing '+fmt$0(owing)+'</h2>'
        +'<a href="#" onclick="go(\'summary\');return false" class="btn ghost small">FY summary →</a></div>'
        +'<div class="cbody">'
        +'<p style="font-size:.88rem;color:var(--muted);margin-bottom:10px">Based on current records you are estimated to owe <b>'+fmt$0(owing)+'</b> at tax time in '+esc(fyDisplay(y))+' — tax payable '+fmt$0(s.tax.total)+' minus '+fmt$0(s.withheld)+' withheld so far.</p>'
        +'<div style="font-size:.85rem;font-weight:var(--fw-semibold);color:var(--ink);margin-bottom:6px">Ways to reduce this:</div>'
        +'<div style="display:flex;flex-direction:column;gap:6px;font-size:.84rem;color:var(--muted)">'
        +(suggestSuper?'<div>💰 <b>Voluntary super contribution</b> — a concessional contribution reduces your taxable income. Check your remaining cap on the <a href="#" onclick="go(\'super\');return false">Superannuation page</a>.</div>':'')
        +'<div>🗂 <b>Claim all deductions</b> — work expenses, WFH days, investment costs. See <a href="#" onclick="go(\'other\');return false">Deductions</a> and the <a href="#" onclick="go(\'wfh\');return false">WFH tracker</a>.</div>'
        +'<div>📋 <b>Verify tax withheld</b> — confirm employer withholding is correct on your <a href="#" onclick="go(\'income\');return false">income records</a>.</div>'
        +'</div></div></div>');
    }
  }

  // ── CGT 2027 Reform — valuation audit reminder ──
  const SHOW_CGT_AUDIT_UNTIL='2028-01-01';
  const todayForAudit=todayISO();
  if(todayForAudit<SHOW_CGT_AUDIT_UNTIL&&!y.locked){
    const cgtAssets=assetsForPerson([pid]).filter(a=>
      (a.kind==='shares'||a.kind==='managed_fund'||a.kind==='property'||a.kind==='vehicle')
      &&!a.closedDate
    );
    const needValuation=cgtAssets.filter(a=>!a.cgt2027Confirmed);
    if(needValuation.length>0){
      const daysToReform=Math.round((new Date(CGT_REFORM_DATE+'T00:00:00Z')-Date.now())/86400000);
      const isAfterReform=todayForAudit>=CGT_REFORM_DATE;
      const urgency=daysToReform<90&&!isAfterReform?'color:var(--red)':daysToReform<180?'color:var(--gold-badge)':'color:var(--muted)';
      const rows=needValuation.map(a=>{
        const isShare=a.kind==='shares'||a.kind==='managed_fund';
        const hasEstimate=a.cgt2027Value&&!a.cgt2027Confirmed;
        const status=hasEstimate
          ?'<span style="color:var(--gold-badge);font-size:.78rem">📝 Estimated: '+fmt$(a.cgt2027Value)+'/unit</span>'
          :'<span style="color:var(--muted);font-size:.78rem">'+( isShare?'No valuation recorded yet':'No valuation recorded yet — professional valuation recommended')+'</span>';
        return '<div style="display:flex;align-items:center;justify-content:space-between;padding:7px 0;border-bottom:1px dashed var(--line);gap:12px">'
          +'<div style="flex:1;min-width:0">'
          +'<div style="font-size:.88rem;font-weight:var(--fw-semibold)">'+esc(a.name)+(a.code?' <span class="badge blue" style="font-size:.68rem">'+esc(a.code)+'</span>':'')+'</div>'
          +status+'</div>'
          +'<button class="btn ghost small" onclick="cgt2027Record(\''+a.id+'\')">Record value</button>'
          +'</div>';
      }).join('');
      const confirmed=cgtAssets.length-needValuation.length;
      const cgtCardKey='ledger-cgt-audit-expanded';
      const isExpanded=localStorage.getItem(cgtCardKey)==='1';
      m.insertAdjacentHTML('beforeend','<div class="card" style="border-color:var(--gold-badge);margin-bottom:18px">'
        +'<div class="chead" style="background:color-mix(in srgb,var(--gold-soft) 40%,var(--surface));cursor:pointer" onclick="(function(el){const b=el.closest(\'.card\').querySelector(\'.cgt-audit-body\');const open=b.style.display!==\'none\';b.style.display=open?\'none\':\'\';localStorage.setItem(\'ledger-cgt-audit-expanded\',open?\'0\':\'1\');el.querySelector(\'.cgt-chev\').textContent=open?\'▾\':\'▴\';})(this)">'
        +'<div>'
        +'<h2 style="color:var(--gold-badge)">📋 CGT Reform — 1 July 2027 valuation audit</h2>'
        +'<div style="font-size:.78rem;color:var(--muted);margin-top:2px">'+(isAfterReform?'Reform commenced':'Reform commences 1 July 2027')
        +' · <span style="'+urgency+'">'+(isAfterReform?'Record valuations as soon as possible':daysToReform+' days away')+'</span>'
        +(confirmed?' · '+confirmed+'/'+cgtAssets.length+' assets confirmed':'')+'</div></div>'
        +'<div style="display:flex;align-items:center;gap:10px">'
        +'<a href="#" onclick="go(\'assets\');return false;event.stopPropagation()" class="btn ghost small">All assets →</a>'
        +'<span class="cgt-chev" style="color:var(--muted);font-size:.85rem;min-width:12px">'+(isExpanded?'▴':'▾')+'</span>'
        +'</div></div>'
        +'<div class="cgt-audit-body cbody" style="display:'+(isExpanded?'':'none')+'">'
        +'<p style="font-size:.84rem;color:var(--muted);margin-bottom:10px">The Government\'s proposed CGT reforms split gains at 1 July 2027 — the 50% discount applies to gains <em>before</em> that date, and cost-base indexation + 30% minimum tax applies to gains <em>after</em> it. To use the transitional rules, you need the market value of each CGT asset on 1 July 2027. Record it here as you obtain it.'
        +(isAfterReform?'':' <b>Legislation has not yet passed</b> — but it\'s worth preparing regardless.')+'</p>'
        +rows
        +'</div></div>');
    }
  }
  ...
  const {start:fyStart}=fyRange(y);
  (b.incomes||[]).forEach(r=>{
    const rec=r.recurrence||'monthly';
    const mult=REC_MULT[rec]||12;
    const periodGross=num(r.yearly)/mult;
    const linked=(b.preTaxDeds||[]).filter(p=>p.incomeId===r.id);
    const periodPreTax=linked.reduce((s,p)=>{const pm=REC_MULT[p.recurrence||'monthly']||12;return s+num(p.yearly)/pm;},0);
    ...
    if(rec==='once'||rec==='yearly'){
      // Non-recurring: single entry on the recorded date
      const d=r.date||'';
      if(d&&d>=fyStart&&d<=todayStr)
        recentItems.push({date:d,desc:esc(r.name)+preLabel,cat:'Income',catCls:'euc',amt:periodNet,sign:1});
    }else{
      // Recurring: walk backwards from today generating one entry per period
      const day=num(r.schedDay)||1;
      const now=new Date();
      let count=0;
      const maxBack=14; // never generate more than 14 entries per source
      // Determine interval in days
      const intervalDays=rec==='weekly'?7:rec==='fortnightly'?14:rec==='quarterly'?91:30;
      // Start from the most-recent past pay date
      let cur=new Date(Date.UTC(now.getUTCFullYear(),now.getUTCMonth(),Math.min(day,28)));
      if(iso(cur)>todayStr)cur=new Date(Date.UTC(cur.getUTCFullYear(),cur.getUTCMonth()-1,Math.min(day,28)));
      while(count<maxBack){
        const d=iso(cur);
        if(d<fyStart)break; // don't go before the FY
        recentItems.push({date:d,desc:esc(r.name)+preLabel,cat:'Income',catCls:'euc',amt:periodNet,sign:1});
        count++;
        // Step back one period
        if(rec==='monthly')cur=new Date(Date.UTC(cur.getUTCFullYear(),cur.getUTCMonth()-1,Math.min(day,28)));
        else cur=new Date(cur.getTime()-intervalDays*86400000);
      }
    }
  });
  ...
  const recentHTML=shown.length
    ?`<div style="overflow-x:auto"><table class="tbl"><thead><tr>
      ...
      ${shown.map(it=>`<tr>
        ...
        <td>${it.desc}</td>
        <td><span class="badge soft ${it.catCls}">${it.cat}</span></td>
  ...
  // tax strip is rendered above the income vs expenses chart
};
...
let BUD_VSACTUAL=null; // id of budget being viewed vs actual spending
const BUD_PER_MULT={w:52,m:12,b:2,y:1};
const BUD_EXP_CATS=['Housing','Utilities','Vehicle Expenses','Food & Groceries','Insurance','Health & Beauty','Entertainment','Sports & Leisure','Subscriptions','Administration','Investments','Education','Other'];
const BUD_EXP_CAT_MAP={'Housing':'Housing','Transport':'Vehicle Expenses','Food & groceries':'Food & Groceries','Utilities':'Utilities','Insurance':'Insurance','Healthcare':'Health & Beauty','Entertainment':'Entertainment','Education':'Education','Clothing':'Other','Subscriptions':'Subscriptions','Donations':'Other','Travel':'Entertainment','Investment / savings':'Investments','Other':'Other'};
function mapExpCatToBudCat(cat){return BUD_EXP_CAT_MAP[cat]||'Other';}
/* yrOf(r, y) — yearly value of a budget row. Pass the FY object (from
   FY()) as the second argument to prorate based on the row's optional
   startDate: if the row only became active partway through the FY (e.g.
   a novated lease starting in October), the yearly figure reflects only
   the fraction of the FY actually active, rather than a flat full-year
   amount. Day-based so it works the same way regardless of frequency or
   custom FY date ranges. Omitting y (or no startDate on the row) keeps
   the original unprorated behaviour, so existing yrOf(r) call sites are
   unaffected. */
const yrOf=(r,y)=>{
  const base=num(r.amt)*(BUD_PER_MULT[r.per]||12);
  if(!y||!r.startDate)return base;
  const{start:fyStart,end:fyEnd}=fyRange(y);
  if(r.startDate>fyEnd)return 0; // not active yet this FY
  const effStart=r.startDate>fyStart?r.startDate:fyStart;
  const totalDays=(new Date(fyEnd+'T00:00:00Z')-new Date(fyStart+'T00:00:00Z'))/86400000+1;
  const activeDays=(new Date(fyEnd+'T00:00:00Z')-new Date(effStart+'T00:00:00Z'))/86400000+1;
  return activeDays<=0?0:base*activeDays/totalDays;
};
...
function defaultOpts(){return{medicare:true,mls:false,hasCover:true,hasPHI:false,phiPremium:0,phiAge:'under65'};}
...
/* All post-tax expenses that count toward a budget's totals: its own rows
   (for a combined budget, genuinely shared costs like joint rent), plus —
   for a combined household budget — every linked individual budget's own
   expense rows too. Individual expenses are read-only from here; edit them
   on the individual budget itself, the same way incomes/deds already work
   via budSourceList. */
function budAllExps(b){
  return b.isCombined?(b.exps||[]).concat(budSourceList(b,'exps')):(b.exps||[]);
}
...
function budgetCompare(m,y){
  ...
  const lineTable=(title,listA,listB)=>{
    ...
    const sumBy=(list,name)=>list.filter(r=>(r.name||'—')===name).reduce((s,r)=>s+yrOf(r,y),0);
  ...
  ${lineTable('Post-tax expenses',budAllExps(A),budAllExps(B))}`);
}

function budVsActual(m,y,b){
  head(m,esc(b.name)+' — vs Actual',
    'Budget plan compared to your recorded expenses in <b>'+esc(fyDisplay(y))+'</b>.',
    '<button class="btn" onclick="BUD_VSACTUAL=null;render()">← All budgets</button> <button class="btn ghost" onclick="BUD_OPEN=\''+b.id+'\';BUD_VSACTUAL=null;render()">Edit budget</button>');
  const pids=b.scope==='all'?DB.people.map(p=>p.id):[b.scope];
  const {start,end}=fyRange(y);
  const actualExps=[];
  pids.forEach(pid=>{
    (bucket(y,pid).expenses||[]).forEach(e=>{
      if(e.recurrence==='once'||e.recurrence==='yearly'){if(e.date&&(e.date<start||e.date>end))return;}
      actualExps.push(e);
    });
  });
  const budExps=budAllExps(b);
  const budByCategory={};
  budExps.forEach(r=>{const cat=r.category||'Other';budByCategory[cat]=(budByCategory[cat]||0)+yrOf(r,y);});
  const actByCategory={};
  actualExps.forEach(e=>{const cat=e.category||'Other';actByCategory[cat]=(actByCategory[cat]||0)+yearlyFromRec(num(e.amount),e.recurrence);});
  const allCats=[...new Set([...Object.keys(budByCategory),...Object.keys(actByCategory)])].sort();
  const light=(actual,budget)=>{
    if(!budget)return '';
    const pct=actual/budget;
    if(pct>1)return '<span style="color:var(--red)">🔴</span>';
    if(pct>0.8)return '<span style="color:var(--gold-badge)">🟡</span>';
    return '<span style="color:var(--euc)">🟢</span>';
  };
  const progBar=(actual,budget)=>{
    if(!budget)return '';
    const pct=Math.min(actual/budget*100,100);const over=actual>budget;
    return '<div style="height:4px;background:var(--line);border-radius:2px;margin-top:4px"><div style="height:4px;border-radius:2px;width:'+pct.toFixed(1)+'%;background:'+(over?'var(--red)':pct>80?'var(--gold-badge)':'var(--euc)')+'"></div></div>';
  };
  const budTotal=Object.values(budByCategory).reduce((s,v)=>s+v,0);
  const actTotal=Object.values(actByCategory).reduce((s,v)=>s+v,0);
  const variance=budTotal-actTotal;
  const rows=allCats.map(cat=>{
    const bud=budByCategory[cat]||0,act=actByCategory[cat]||0,diff=bud-act;
    const diffStyle=diff<0?'color:var(--red)':diff>0?'color:var(--euc)':'';
    return '<tr><td>'+esc(cat)+'</td><td>'+light(act,bud||null)+'</td>'
      +'<td class="num">'+(bud?fmt$(bud/12):'<span class="muted">—</span>')+'</td>'
      +'<td class="num">'+(act?fmt$(act/12):'<span class="muted">—</span>')+'</td>'
      +'<td class="num">'+(bud?fmt$0(bud):'<span class="muted">—</span>')+'</td>'
      +'<td class="num">'+fmt$0(act)+'</td>'
      +'<td class="num" style="'+diffStyle+'">'+(bud?(diff===0?'—':(diff>0?'+':'')+fmt$0(diff)):'<span class="muted">—</span>')+'</td></tr>'
      +(bud?'<tr><td colspan="7" style="padding:0 16px 8px;border-top:none">'+progBar(act,bud)+'</td></tr>':'');
  }).join('');
  const totRow='<tr class="total"><td><b>Total</b></td><td></td>'
    +'<td class="num">'+fmt$(budTotal/12)+'</td><td class="num">'+fmt$(actTotal/12)+'</td>'
    +'<td class="num">'+fmt$0(budTotal)+'</td><td class="num">'+fmt$0(actTotal)+'</td>'
    +'<td class="num" style="'+(variance<0?'color:var(--red)':variance>0?'color:var(--euc)':'')+'">'+
      (variance===0?'—':(variance>0?'+':'')+fmt$0(variance))+'</td></tr>';
  m.insertAdjacentHTML('beforeend','<div class="card">'
    +'<div class="chead"><h2>Expenses — Budget vs Actual</h2>'
    +'<span class="hint">🟢 on track &nbsp; 🟡 >80% used &nbsp; 🔴 over budget</span></div>'
    +'<div class="cbody tight"><table class="tbl">'
    +'<thead><tr><th>Category</th><th></th><th class="num">Budget/mo</th><th class="num">Actual/mo</th><th class="num">Budget/yr</th><th class="num">Actual/yr</th><th class="num">Variance</th></tr></thead>'
    +'<tbody>'+(rows||'<tr><td colspan="7" class="muted">No expenses in either the budget or your records for '+esc(fyDisplay(y))+'.</td></tr>')+(rows?totRow:'')+'</tbody>'
    +'</table></div>'
    +(!budExps.length?'<div class="cbody" style="padding-top:0"><div class="note">No expense rows yet in this budget — <a href="#" onclick="BUD_OPEN=\''+b.id+'\';BUD_VSACTUAL=null;render();return false">open it</a> to add some.</div></div>':'')
    +'</div>'
    +'<div class="note">Actuals are expenses on the <a href="#" onclick="go(\'expenses\');return false">Expenses page</a> for '+(pids.length>1?'the people in this budget':'this person')+' in '+esc(fyDisplay(y))+'. Recurring expenses are annualised; one-off expenses only count if dated within this FY.</div>');
}
PAGES.budget=m=>{
  const y=FY();
  ...
  if(BUD_VSACTUAL){const b=budGet(BUD_VSACTUAL);if(b)return budVsActual(m,y,b);}
  ...
  m.insertAdjacentHTML('beforeend',`<div class="note" style="margin-bottom:14px">This is a what-if planning space, separate from your actual records. Income, expenses, and deductions entered in a budget don't affect your real Income / Expenses / Deductions pages or your tax return — use <b>"Create from FY data"</b> to start a budget pre-filled from what you've actually recorded this year.</div>`);
  ...
  const rows=vis.map(b=>{
    ...
    <td class="rowact"><button class="btn ghost small" onclick="BUD_OPEN='${b.id}';render()">Open</button>
      <button class="btn ghost small" onclick="BUD_VSACTUAL='${b.id}';render()" title="Compare planned budget to actual recorded expenses">vs Actual</button>
...
function budFromFY(){
  ...
  pids.forEach(pid=>{
    ...
    (bkt.preTaxDeds||[]).forEach(r=>{
      b.deds.push({id:uid(),name:r.name,amt:preTaxDedYearlyFor(r,y),per:'y',pid});
    });
    ...
    Object.entries(expCats).forEach(([cat,amt])=>{
      if(amt>0)b.exps.push({id:uid(),name:cat,amt:round2(amt),per:'y',category:mapExpCatToBudCat(cat)});
    });
    // Property expenses as a living expense
    const propExp=(bkt.property?.expenses||[]).reduce((s,e)=>s+num(e.amount),0);
    if(propExp)b.exps.push({id:uid(),name:'Property expenses (incl. mortgage)',amt:round2(propExp),per:'y',category:'Housing'});
  });
...
function budIncomeTaxable(r,sgRate,y){
  const yearly=yrOf(r,y);
  return r.includesSuper?yearly/(1+(sgRate||11.5)/100):yearly;
}
/* The tax-deductible portion of a budget expense, prorated the same way
   as yrOf — e.g. work-related phone use claimed at 60%, or the
   deductible interest portion of an investment property cost. Fixed-$
   mode is expressed as a virtual row sharing the same per/startDate so
   proration applies identically to the deductible slice as to the
   expense itself. */
function budExpDeductibleAmt(r,y){
  if(!r.isDeductible)return 0;
  if(r.deductibleMode==='percent')return yrOf(r,y)*num(r.deductiblePct)/100;
  return yrOf({amt:r.deductibleAmount,per:r.per,startDate:r.startDate},y);
}
function budTotals(y,b){
  const pids=b.scope==='all'?DB.people.map(p=>p.id):[b.scope];
  const incomes=budSourceList(b,'incomes'),deds=budSourceList(b,'deds');
  const exps=budAllExps(b);
  const sgRate=y.rates.superSGRate||11.5;
  // Deductible expenses reduce taxable income, attributed to whichever
  // person is set to claim them (dedPid) — falling back to the row's own
  // owner (r.pid, always correctly tagged on inherited rows by
  // budSourceList) and only then to the budget's first/only person. Without
  // the r.pid fallback, an inherited expense from a combined budget's
  // second/third household member with no dedPid set would silently get
  // attributed to whichever person happens to be first in the household.
  const dedExpsByPid={};
  exps.forEach(r=>{
    const amt=budExpDeductibleAmt(r,y);
    if(amt>0){const pid=r.dedPid||r.pid||pids[0];dedExpsByPid[pid]=(dedExpsByPid[pid]||0)+amt;}
  });
  let gross=0,tax=0,perP={};
  pids.forEach(pid=>{
    const g=incomes.filter(r=>(r.pid||pids[0])===pid).reduce((s,r)=>s+budIncomeTaxable(r,sgRate,y),0);
    const d=deds.filter(r=>(r.pid||pids[0])===pid).reduce((s,r)=>s+yrOf(r,y),0);
    const dedExps=dedExpsByPid[pid]||0;
    const taxable=Math.max(0,g-d-dedExps);
    ...
    perP[pid]={gross:g,deds:d,dedExps,taxable,t,opts};gross+=g;tax+=t.total;
  });
  const expsTotal=exps.reduce((s,r)=>s+yrOf(r,y),0);
  const dedExpsTotal=Object.values(dedExpsByPid).reduce((s,v)=>s+v,0);
  const dedsTotal=budSourceList(b,'deds').reduce((s,r)=>s+yrOf(r,y),0);
  // net = gross minus ALL pre-tax deductions (that money is gone — it goes
  // to salary sacrifice, novated lease, purchased leave, etc.) minus tax.
  // Previously this was gross-tax which incorrectly left pre-tax sacrifice
  // money still counted as available take-home.
  const net=gross-dedsTotal-tax, excess=net-expsTotal;
  return{pids,perP,gross,dedsTotal,tax,exps:expsTotal,dedExpsTotal,net,excess};
}
function budRows(b,list,kind,withPerson,y){
  ...
  const rowHtml=(r,i)=>{
    ...
    // Only show a "from [date]" hint when the start date actually falls
    // within this FY (i.e. genuinely causes proration) — a start date
    // from a prior FY is already fully active, so showing it would just
    // be clutter.
    if(r.startDate&&y){
      const{start:fyStart,end:fyEnd}=fyRange(y);
      if(r.startDate>fyStart&&r.startDate<=fyEnd)schedDetail+=` · from ${fmtDate(r.startDate)}`;
    }
    const yearly=yrOf(r,y),monthly=yearly/12;
    const pp=withPerson?person(r.pid):null;
    const dedAmt=kind==='exps'?budExpDeductibleAmt(r,y):0;
    // For linked leave rows, show the link source inline
    let linkHint='';
    if(kind==='deds'&&r.linkedIncomeId){
      const incRow=budSourceList(b,'incomes').find(x=>x.id===r.linkedIncomeId);
      if(incRow){
        const wkRate=yrOf(incRow,y)/52;
        linkHint=` <span class="muted" style="font-size:.73rem">🔗 ${esc(incRow.name)} · ${r.linkedLeaveWeeks}wk × ${fmt$(wkRate)}/wk</span>`;
      }
    }
    return `<tr>
    <td><b>${esc(r.name)||'<span class="muted">Unnamed</span>'}</b>${kind==='incomes'&&r.includesSuper?' <span class="badge gold" style="font-size:.68rem">incl. super</span>':''}${dedAmt>0?` <span class="badge euc" style="font-size:.68rem">✓ ${fmt$0(dedAmt)}/yr deductible</span>`:''}${linkHint}
  ...
  if(kind!=='exps')return list.map(rowHtml).join('');
  // Expenses: group by category, in the standard category order, with
  // any custom/legacy category not in that list appended at the end.
  const byCat={};
  list.forEach((r,i)=>{const c=r.category||'Other';(byCat[c]=byCat[c]||[]).push([r,i]);});
  const totalCols=withPerson?5:4;
  const order=BUD_EXP_CATS.concat(Object.keys(byCat).filter(c=>!BUD_EXP_CATS.includes(c)));
  let html='';
  order.forEach(cat=>{
    if(!byCat[cat])return;
    const catYearly=byCat[cat].reduce((s,[r])=>s+yrOf(r,y),0);
    html+=`<tr class="subhead"><td colspan="${totalCols}">${esc(cat)} — ${fmt$0(catYearly)}/yr</td></tr>`;
    byCat[cat].forEach(([r,i])=>{html+=rowHtml(r,i);});
  });
  return html;
}
...
const BUD_PERIOD_LABEL={w:'week',m:'month',b:'half-year',y:'year'};
function budMult(per){return per==='w'?52:per==='b'?2:per==='y'?1:12;}
/* Add/edit a budget row via modal — name, amount, frequency, and the
   ...
   The amount field has a "per year / per [frequency]" mode toggle, same
   as the Income page's salary entry — type the yearly figure and it
   converts to the per-frequency amount actually stored, or vice versa. */
function budRowAdd(budId,kind,i){
  if(lockedGuard())return;
  const b=budGet(budId);if(!b)return;
  const pids=b.scope==='all'?DB.people.map(p=>p.id):[b.scope];
  const withPerson=(kind==='incomes'||kind==='deds')&&b.scope==='all';
  const r=i!=null?b[kind][i]:{name:'',amt:0,per:'m',pid:pids[0],schedDay:'',includesSuper:false,category:'Other'};
  const per=r.per||'m';
  const dedMode=r.deductibleMode||'fixed';
  const kindLabel=kind==='incomes'?'income':kind==='deds'?'deduction':'expense';
  // Income-link options for purchased-leave deductions
  const incomeRows=budSourceList(b,'incomes');
  const leaveLink=kind==='deds'?`
    <hr style="border:none;border-top:1px solid var(--line);margin:14px 0">
    <label style="display:flex;gap:8px;align-items:center;margin-bottom:10px;cursor:pointer">
      <input type="checkbox" id="bf_leaveLink" ${r.linkedIncomeId?'checked':''} onchange="budRowLeaveLinkToggle('${budId}')">
      <span>Link to an income source <span class="muted">(auto-calculate from weekly rate)</span></span>
    </label>
    <div id="bf_leaveLinkFlds" style="display:${r.linkedIncomeId?'block':'none'}">
      <div class="fldrow">
        <div><label class="fld">Income source</label>
          <select id="bf_leaveIncome" class="input" onchange="budRowLeaveRecalc('${budId}')">
            <option value="">— choose income —</option>
            ${incomeRows.map(inc=>`<option value="${inc.id}" ${r.linkedIncomeId===inc.id?'selected':''}>${esc(inc.name)} (${fmt$0(yrOf(inc,FY()))}/yr)</option>`).join('')}
          </select>
        </div>
        <div><label class="fld">Weeks purchased</label>
          <input id="bf_leaveWeeks" class="input money" value="${r.linkedLeaveWeeks||1}" min="0.5" step="0.5" oninput="budRowLeaveRecalc('${budId}')">
        </div>
      </div>
      <div id="bf_leaveCalcNote" class="note" style="margin-top:6px"></div>
    </div>`:'';
  modal(i!=null?`Edit ${kindLabel}`:`Add ${kindLabel}`,`
    ...
    ${kind==='exps'?`<div><label class="fld">Category</label><select id="bf_cat" class="input">${BUD_EXP_CATS.map(c=>`<option ${(r.category||'Other')===c?'selected':''}>${c}</option>`).join('')}</select></div>`:''}
    ...
    <div class="fldrow mt">
      <div style="flex:2"><label class="fld">Amount ($) <span class="muted" id="bf_amtHint"></span></label>
        <div style="display:flex;gap:8px">
          <input id="bf_amt" class="input money" value="${r.amt}" oninput="budRowAmountPreview()" style="flex:1">
          <select id="bf_amode" class="input" style="width:auto" onchange="budRowAmountModeChange()" data-prev="period">
            <option value="period">per ${BUD_PERIOD_LABEL[per]}</option>
            <option value="yearly">per year</option>
          </select>
        </div>
      </div>
      <div><label class="fld">Frequency</label><select id="bf_per" class="input" onchange="budRowFreqChange()" data-prev="${per}">
    ...
    <div class="fldrow mt">
      <div><label class="fld" id="bf_schedLabel">${per==='w'?'Day of week':per==='m'?'Day of month':'Month'}</label>
        <div id="bf_schedField">${budSchedFieldHtml(per,r.schedDay)}</div></div>
      <div><label class="fld">Start date <span class="muted">(optional)</span></label><input id="bf_startDate" type="date" class="input" value="${r.startDate||''}" title="If this only became active partway through the year, the yearly figure shown will be prorated from this date"></div>
    </div>
    ...
    ${kind==='exps'?`<hr style="border:none;border-top:1px solid var(--line);margin:14px 0">
    <label style="display:flex;gap:8px;align-items:center;margin-bottom:10px;cursor:pointer"><input type="checkbox" id="bf_ded" ${r.isDeductible?'checked':''} onchange="budRowDedToggle()"> <span>This expense has a <b>tax-deductible portion</b></span></label>
    <div id="bf_dedFlds" style="display:${r.isDeductible?'block':'none'}">
      <div class="fldrow">
        <div><label class="fld">Claimable as</label><select id="bf_dmode" class="input" onchange="budRowDedModeToggle()">
          <option value="fixed" ${dedMode==='fixed'?'selected':''}>Fixed $ amount</option>
          <option value="percent" ${dedMode==='percent'?'selected':''}>% of the amount</option>
        </select></div>
        <div id="bf_dedFixedFld" style="display:${dedMode==='fixed'?'block':'none'}"><label class="fld" id="bf_dedFixedLbl">Claimable amount ($ per ${BUD_PERIOD_LABEL[per]})</label><input id="bf_damt" class="input money" value="${r.deductibleAmount||''}" placeholder="e.g. 50"></div>
        <div id="bf_dedPctFld" style="display:${dedMode==='percent'?'block':'none'}"><label class="fld">Claimable %</label><input id="bf_dpct" class="input money" value="${r.deductiblePct||''}" placeholder="e.g. 80"></div>
      </div>
      ${pids.length>1?`<div class="mt"><label class="fld">Claimed by</label><select id="bf_dedPid" class="input">${pids.map(pid=>`<option value="${pid}" ${(r.dedPid||pids[0])===pid?'selected':''}>${esc(person(pid).name)}</option>`).join('')}</select></div>`:''}
      <div class="note mt">The deductible portion reduces taxable income in this budget's tax calculation — the full expense amount still counts as real cash spent.</div>
    </div>`:''}
    ${leaveLink}`,
  ...
  budRowAmountPreview();
  if(r.linkedIncomeId)setTimeout(()=>budRowLeaveRecalc(budId),50);
}
function budRowDedToggle(){
  const f=$('#bf_dedFlds');if(f)f.style.display=$('#bf_ded').checked?'block':'none';
}
function budRowLeaveLinkToggle(budId){
  const flds=$('#bf_leaveLinkFlds');
  if(flds)flds.style.display=$('#bf_leaveLink').checked?'block':'none';
  if($('#bf_leaveLink').checked)budRowLeaveRecalc(budId);
}
function budRowLeaveRecalc(budId){
  const b=budGet(budId);if(!b)return;
  const incId=$('#bf_leaveIncome')?.value;
  const weeks=num($('#bf_leaveWeeks')?.value)||0;
  const note=$('#bf_leaveCalcNote');
  if(!incId||!weeks){if(note)note.textContent='';return;}
  const incRow=budSourceList(b,'incomes').find(r=>r.id===incId);
  if(!incRow){if(note)note.textContent='';return;}
  const yearly=yrOf(incRow,FY());
  const weeklyRate=yearly/52;
  const leaveAmt=round2(weeklyRate*weeks);
  // Set amount field to the calculated yearly total and switch to yearly mode
  const amtEl=$('#bf_amt'),amodeEl=$('#bf_amode');
  if(amtEl){amtEl.value=leaveAmt;}
  if(amodeEl){
    amodeEl.value='yearly';amodeEl.dataset.prev='yearly';
    const opt=amodeEl.querySelector('option[value="period"]');
    if(opt)opt.textContent='per '+BUD_PERIOD_LABEL[($('#bf_per')||{}).value||'m'];
  }
  if(note)note.innerHTML=`<b>${weeks} week${weeks!==1?'s':''}</b> × ${fmt$(weeklyRate)}/wk = <b>${fmt$0(leaveAmt)}/yr</b> — reduces taxable income by this amount via salary sacrifice.`;
  budRowAmountPreview();
}
function budRowDedModeToggle(){
  const mode=$('#bf_dmode').value;
  const fixedFld=$('#bf_dedFixedFld'),pctFld=$('#bf_dedPctFld');
  if(fixedFld)fixedFld.style.display=mode==='fixed'?'block':'none';
  if(pctFld)pctFld.style.display=mode==='percent'?'block':'none';
}
function budRowAmountPreview(){
  const hint=$('#bf_amtHint');if(!hint)return;
  const amt=num($('#bf_amt').value),mode=$('#bf_amode').value,per=$('#bf_per').value;
  if(!amt){hint.textContent='';return;}
  if(mode==='period')hint.textContent=per!=='y'?`= ${fmt$0(amt*budMult(per))}/yr`:'';
  else hint.textContent=per!=='y'?`= ${fmt$0(amt/budMult(per))}/${BUD_PERIOD_LABEL[per]}`:'';
}
function budRowAmountModeChange(){
  const sel=$('#bf_amode'),amt=num($('#bf_amt').value),per=$('#bf_per').value;
  const prevMode=sel.dataset.prev||'period',newMode=sel.value;
  const yearly=prevMode==='period'?amt*budMult(per):amt;
  $('#bf_amt').value=round2(newMode==='period'?yearly/budMult(per):yearly);
  sel.dataset.prev=newMode;
  budRowAmountPreview();
}
function budRowFreqChange(){
  const perSel=$('#bf_per'),per=perSel.value,prevPer=perSel.dataset.prev||'m';
  perSel.dataset.prev=per;
  // Schedule sub-field (day of week/month, or month-of-year)
  ...
  // Keep the amount meaningful across a frequency change: convert to
  // yearly using the OLD frequency's multiplier, then back to the
  // current mode using the NEW frequency's multiplier (same approach as
  // the Income page's recurrence change).
  const amode=$('#bf_amode');
  if(amode){
    const opt=amode.querySelector('option[value="period"]');
    if(opt)opt.textContent='per '+BUD_PERIOD_LABEL[per];
    const amt=num($('#bf_amt').value);
    if(amode.value==='period'){
      const yearly=amt*budMult(prevPer);
      $('#bf_amt').value=round2(yearly/budMult(per));
    }
  }
  const dedLbl=$('#bf_dedFixedLbl');
  if(dedLbl)dedLbl.textContent=`Claimable amount ($ per ${BUD_PERIOD_LABEL[per]})`;
  budRowAmountPreview();
}
function budRowSave(budId,kind,i){
  ...
  const typedAmt=num($('#bf_amt').value);
  const mode=$('#bf_amode')?$('#bf_amode').value:'period';
  const amt=mode==='yearly'?round2(typedAmt/budMult(per)):typedAmt;
  const r={id:prev?prev.id:uid(),name:$('#bf_n').value.trim()||'Untitled',amt,per,schedDay:$('#bf_sched')?$('#bf_sched').value:'',startDate:$('#bf_startDate')?.value||''};
  if(kind!=='exps'){
    if($('#bf_pid'))r.pid=$('#bf_pid').value;else r.pid=(prev&&prev.pid)||pids[0];
  }
  if(kind==='incomes')r.includesSuper=!!$('#bf_super')?.checked;
  if(kind==='deds'){
    // Leave link: if checked, store the linked income id and weeks so the
    // link is preserved when the row is edited again later
    if($('#bf_leaveLink')?.checked&&$('#bf_leaveIncome')?.value){
      r.linkedIncomeId=$('#bf_leaveIncome').value;
      r.linkedLeaveWeeks=num($('#bf_leaveWeeks')?.value)||1;
    }else{
      r.linkedIncomeId=null;r.linkedLeaveWeeks=null;
    }
  }
  if(kind==='exps'){
    r.category=$('#bf_cat')?.value||'Other';
    r.isDeductible=!!$('#bf_ded')?.checked;
    if(r.isDeductible){
      r.deductibleMode=$('#bf_dmode')?.value||'fixed';
      r.deductibleAmount=num($('#bf_damt')?.value);
      r.deductiblePct=num($('#bf_dpct')?.value);
      r.dedPid=$('#bf_dedPid')?.value||(prev&&prev.dedPid)||pids[0];
    }
  }
...
function budgetEditor(m,y,b){
  ...
  head(m,esc(b.name),`${budScopeLabel(b)}${combined?' · live-linked to individual budgets below':''} · saved in ${esc(fyDisplay(y))} — edits stick, and the ${esc(y.label)} rate snapshot does the tax. <span class="muted">Planning only — separate from your actual Income / Expenses / Deductions records.</span>`,
  ...
  const incomes=budSourceList(b,'incomes'),deds=budSourceList(b,'deds'),indivExps=budSourceList(b,'exps');
  let incCard,dedCard,expIndivCard,optBlock;
  if(combined){
    const lineRows=(list)=>list.map(r=>{const dedAmt=r.isDeductible?budExpDeductibleAmt(r,y):0;return `<tr><td>${esc(r.name||'—')}${dedAmt>0?` <span class="badge euc" style="font-size:.68rem">✓ ${fmt$0(dedAmt)}/yr deductible</span>`:''}</td><td>${pdot(person(r.pid))} ${esc(person(r.pid).name)}</td><td class="num">${fmt$(yrOf(r,y))}</td></tr>`;}).join('');
    ...
    expIndivCard=`<div class="card"><div class="chead"><h2>Individual expenses</h2><span class="hint">read-only — from individual budgets</span></div>
      <div class="cbody tight"><table class="tbl"><thead><tr><th>Expense</th><th>Person</th><th class="num">Yearly</th></tr></thead>
      <tbody>${lineRows(indivExps)||'<tr><td colspan="3" class="muted">No expense rows in the selected individual budgets.</td></tr>'}</tbody></table></div>
      <div class="cbody" style="padding-top:0"><div class="hint">Each person's own expenses, counted automatically toward the household total below — edit them on that person's individual budget.</div></div></div>`;
    ...
    incCard=`<div class="card"><div class="chead"><h2>Gross income</h2><button class="btn small" onclick="budRowAdd('${b.id}','incomes')">+ Add</button></div>
      ...
      <tbody>${budRows(b,b.incomes,'incomes',hh,y)||`<tr><td colspan="${hh?4:3}" class="muted">No income sources yet — add your salary or other income.</td></tr>`}</tbody></table></div></div>`;
    dedCard=`<div class="card"><div class="chead"><h2>Pre-tax deductions</h2><button class="btn small" onclick="budRowAdd('${b.id}','deds')">+ Add</button></div>
      ...
      <tbody>${budRows(b,b.deds,'deds',hh,y)||`<tr><td colspan="${hh?4:3}" class="muted">None — e.g. investment property, novated lease, super contributions, WFH.</td></tr>`}</tbody></table></div>
      <div class="cbody" style="padding-top:0"><div class="hint">e.g. investment property, novated lease, super contributions, WFH — these reduce taxable income before tax is calculated.</div></div></div>`;
    optBlock=pids.map(pid=>{
  ...
  <div class="grid2">
    <div>
      ${incCard}
      ${dedCard}
      <div class="card"><div class="chead"><h2>${combined?'Shared expenses':`Post-tax expenses ${hh?'(shared)':''}`}</h2><button class="btn small" onclick="budRowAdd('${b.id}','exps')">+ Add</button></div>
      ${combined?`<div class="cbody" style="padding-top:0;padding-bottom:0"><div class="hint">Costs that aren't already in either person's individual budget — e.g. joint rent or a shared mortgage. Each person's own expenses are added automatically below.</div></div>`:''}
      ...
      <tbody>${budRows(b,b.exps,'exps',false,y)||'<tr><td colspan="3" class="muted">No expenses yet — add rent/mortgage, groceries, utilities, and so on.</td></tr>'}</tbody></table></div></div>
      ${expIndivCard}
    </div>
    <div>
  ...
  const incMo=budMonthlyFlow(incomes,y);
  const allExpsB=budAllExps(b);
  const expMo=budMonthlyFlow(allExpsB,y);
  const cashData=FY_MONTHS.map((label,i)=>({label,value:incMo[i]||0,value2:expMo[i]||0}));
  const yearlyInc=incomes.reduce((s,r)=>s+yrOf(r,y),0);
  const yearlyExpB=allExpsB.reduce((s,r)=>s+yrOf(r,y),0);
...
function renderBudgetCalc(){
  ...
  const incomes=budSourceList(b,'incomes'),deds=budSourceList(b,'deds'),allExps=budAllExps(b);
  ...
  r.pids.forEach(pid=>{
    ...
    const incSubs=incomes.filter(q=>(q.pid||r.pids[0])===pid).map(q=>({n:q.name||'Income',v:yrOf(q,y)}));
    const dedSubs=deds.filter(q=>(q.pid||r.pids[0])===pid).map(q=>({n:q.name||'Deduction',v:-yrOf(q,y)}));
    rows+=grp(pfx+'g','Gross income',x.gross,incSubs);
    rows+=grp(pfx+'d','Pre-tax deductions',-x.deds,dedSubs);
    if(x.dedExps>0){
      const dedExpSubs=allExps.filter(q=>q.isDeductible&&(q.dedPid||q.pid||r.pids[0])===pid&&budExpDeductibleAmt(q,y)>0).map(q=>({n:q.name||'Expense',v:-budExpDeductibleAmt(q,y)}));
      rows+=grp(pfx+'de','Deductible expenses',-x.dedExps,dedExpSubs);
    }
    ...
    rows+=plain('Net income',x.gross-x.deds-x.t.total,{c:'color:var(--euc)'}); // gross minus what was sacrificed minus tax
  });
  if(hh)rows+=plain('<b>Household gross income</b>',r.gross);
  if(hh)rows+=plain('<b>Household net income</b>',r.net,{c:'color:var(--euc)'}); // = gross - all pre-tax sacrifice - tax
  const expSubs=allExps.map(q=>({n:q.name||'Expense',v:-yrOf(q,y)}));
  ...
  // Donut: one slice per expense CATEGORY (not per individual row) so it
  // stays readable even with many line items in the same category.
  const segCats={};
  allExps.forEach(x=>{
    const v=yrOf(x,y);
    if(v>0){const cat=x.category||'Other';segCats[cat]=(segCats[cat]||0)+v;}
  });
  const segs=Object.entries(segCats).map(([n,v])=>({n,v}));
...
PAGES.income=m=>{
  ...
  const incRows=B.incomes.map((r,i)=>{
    ...
    // Linked pre-tax deductions for this income
    const linked=(B.preTaxDeds||[]).filter(p=>p.incomeId===r.id);
    const linkedYearly=linked.reduce((s,p)=>s+preTaxDedYearlyFor(p,y),0);
    const netYearly=num(r.yearly)-linkedYearly;
    const alreadyNet=r.alreadyNetOfSacrifice&&linked.length>0;
    // If income is already post-sacrifice, show a note instead of sub-rows
    const linkedSubRows=alreadyNet?'':(linked.map(p=>{
      const fyAmt=preTaxDedYearlyFor(p,y);
      const isProrated=fyAmt!==num(p.yearly)&&p.recurrence!=='once';
      return `<tr style="background:color-mix(in srgb,var(--surface2) 40%,transparent)">
        <td style="padding-left:28px;border-top:none"><span style="color:var(--muted);font-size:.8rem">↳ ${esc(p.name)}</span>
          <div class="muted" style="font-size:.72rem">${esc(p.type)} · pre-tax sacrifice${isProrated?' · *prorated':''}</div></td>
        <td></td>
        <td class="num" style="color:var(--muted);font-size:.85rem">${fmt$(fyAmt/12)}</td>
        <td class="num" style="color:var(--muted);font-size:.85rem">−${fmt$(fyAmt)}</td>
        <td class="num muted">—</td><td></td></tr>`;
    }).join(''));
    const netRow=(!alreadyNet&&linked.length)?`<tr style="background:color-mix(in srgb,var(--surface2) 40%,transparent);border-bottom:2px solid var(--line2)">
        <td style="padding-left:28px;border-top:none"><span style="font-size:.82rem;font-weight:var(--fw-semibold)">Net after sacrifice</span></td>
        <td></td>
        <td class="num" style="font-size:.85rem;font-weight:var(--fw-semibold)">${fmt$(netYearly/12)}</td>
        <td class="num" style="font-size:.85rem;font-weight:var(--fw-semibold)">${fmt$(netYearly)}</td>
        <td class="num muted">—</td><td></td></tr>`:'';
    const netBadge=alreadyNet?` <span class="badge blue" style="font-size:.65rem" title="Amount already excludes ${esc(linked.map(p=>p.name).join(', '))}">post-sacrifice</span>`:'';
    return `<tr>
    <td><b>${esc(r.name)}</b>${r.attractsSuper?` <span class="badge gold" style="font-size:.68rem">SG ${sgRate}%</span>`:''}${netBadge}
    ...
    <button class="btn ghost small" onclick="if(!lockedGuard())confirmDel('Delete ${esc(r.name)}?',()=>{incomeDeleteRow(${i})})">\u2715</button></td></tr>${linkedSubRows}${netRow}`;
  }).join('');
  ...
  const ptdRows=ptd.map((r,i)=>{
    ...
    const fyYearly=preTaxDedYearlyFor(r,FY());
    const isProrated=fyYearly!==num(r.yearly)&&r.recurrence!=='once';
    return `<tr>
    <td><b>${esc(r.name)}</b><div class="muted" style="font-size:.74rem">${esc(r.type)} · ${RECURRENCE_OPTS.find(([v])=>v===(r.recurrence||'monthly'))?.[1]||'Monthly'}${r.schedDay?' · day '+r.schedDay:''}${r.date?' · '+fmtDate(r.date):''}${r.startDate&&r.recurrence!=='once'?' · from '+fmtDate(r.startDate):''}${veh?` · 🚗 ${esc(veh.name)}`:''}${r.incomeId?(()=>{const inc=B.incomes.find(x=>x.id===r.incomeId);return inc?` · <span class="badge blue" style="font-size:.65rem">↳ ${esc(inc.name)}</span>`:''})():''}</div>${led?`<div style="font-size:.74rem;margin-top:2px;color:${leaseExpired?'var(--red)':leaseSoon?'var(--gold)':'var(--muted)'}">🗓 Lease ends ${fmtDate(led)} · ${leaseCountdown(led)}${r.residualAmount?` · residual ${fmt$0(r.residualAmount)}`:''}${leaseExpired?' ⚠ expired':leaseSoon?' ⚠ ending soon':''}</div>`:''}</td>
    <td class="num">${r.recurrence==='once'?'<span class="muted">one-off</span>':fmt$(fyYearly/12)}</td>
    <td class="num">${fmt$(fyYearly)}${isProrated?` <span class="muted" style="font-size:.72rem" title="Prorated from ${fmtDate(r.startDate||r.leaseStartDate)} — full year would be ${fmt$(r.yearly)}">*prorated</span>`:''}</td>
  ...
  const ptdTot=ptd.reduce((s,r)=>s+preTaxDedYearlyFor(r,FY()),0);
...
function barChartSVGClickable(data,opts){
  ...
  data.forEach((d,i)=>{
    ...
    const click2Attr=d.onClick2?` onclick="${d.onClick2}" style="cursor:pointer" tabindex="0" role="button" aria-label="${esc(d.label)} expenses: ${fmt$(v2)}"`:'' ;
    if(i%2===1)bars+=`<rect x="${padL+i*bw}" y="${padT}" width="${bw}" height="${H-padT-padB}" fill="var(--line2)" opacity=".22"/>`;
    if(v2)bars+=`<g${click2Attr}><rect x="${x}" y="${H-padB-h1-h2}" width="${w}" height="${h2}" rx="2" fill="var(--gold)" opacity=".85"><title>${esc(d.label)}: ${fmt$(v2)}</title></rect></g>`;
...
function incRecChange(v){
  ...
  const ptdEl=$('#ptdStartDateRow');if(ptdEl)ptdEl.style.display=show?'none':'flex';
...
function preTaxDedAdd(i){
  ...
  modal(i!=null?'Edit pre-tax deduction':'Add pre-tax deduction',`
    ...
    <div id="incDateRow" class="fldrow" style="display:${(rec==='once'||rec==='yearly')?'flex':'none'};flex:2"><div><label class="fld">Payment date</label><input id="f_dt" type="date" class="input" value="${r.date||''}"></div></div>
    <div id="ptdStartDateRow" style="display:${(rec==='once'||rec==='yearly')?'none':'flex'};flex:2"><div><label class="fld">Start date <span class="muted">(optional — prorates yearly total if mid-FY)</span></label><input id="f_ptdstart" type="date" class="input" value="${r.startDate||r.leaseStartDate||''}" title="If this salary sacrifice started partway through the year, enter the date it began — the yearly amount will be prorated accordingly"></div></div></div>
...
function incomeAdd(i){
  ...
  modal(i!=null?'Edit income':'Add income',`
    ...
    <label class="mt" style="display:flex;gap:8px;align-items:center;cursor:pointer">
      <input type="checkbox" id="f_netOfSacrifice" ${r.alreadyNetOfSacrifice?'checked':''}>
      <span>Amount already excludes salary sacrifice <span class="muted">(my payment summary / ATO income statement shows the post-sacrifice figure)</span></span>
    </label>
...
function incomeSave(i){
  ...
  const r={id:prev?prev.id:uid(),name:$('#f_n').value.trim()||'Income',kind:$('#f_k').value,yearly,
    ...
    alreadyNetOfSacrifice:!!($('#f_netOfSacrifice')?.checked)};
...
function fpSave(i){
  const B=PD();
  const code=$('#f_c').value.trim().toUpperCase();
  if(!code)return toast('Enter a fund code first');
  const p={id:i!=null?B.fundPayments[i].id:uid(),code,date:$('#f_d').value,amount:num($('#f_a').value)};
...
function fundSave(i){
  ...
  const assetId=$('#f_mfaid')?$('#f_mfaid').value:'';
  const linkedAsset=assetId?DB.assets.find(a=>a.id===assetId):null;
  // Prefer the live-selected asset's code/name over the hidden fields — the
  // hidden fields only update via the dropdown's onchange (mfAssetPick),
  // which never fires when there's exactly one fund asset and it's
  // pre-selected by default with no user interaction. That left the hidden
  // fields empty and silently saved '???' as the code.
  const code=(linkedAsset?.code||$('#f_code').value.trim()).toUpperCase();
  const name=linkedAsset?.name||$('#f_name').value.trim();
  if(!code)return toast('Pick a fund / ETF asset first');
  const f={id:i!=null?PD().funds[i].id:uid(),code,name,platform:$('#f_plat').value.trim(),labels,assetId};
...
function saleSave(i){
  const code=$('#f_c').value.trim().toUpperCase();
  if(!code)return toast('Enter a security code first');
  const s={id:i!=null?PD().sales[i].id:uid(),code,buyDate:$('#f_b').value,sellDate:$('#f_s').value,costBase:num($('#f_cb').value),proceeds:num($('#f_pr').value)};
...
PAGES.expenses=m=>{
  ...
  // ── Section helper ──
  const sectionTable=(title,hint,rows,total,emptyMsg,extraClass='')=>{
    if(!rows&&!emptyMsg)return '';
    return `<div class="card ${extraClass}" style="margin-bottom:18px">
      <div class="chead"><h2>${title}</h2>${hint?`<span class="hint">${hint}</span>`:''}
        <span class="muted" style="font-size:.83rem;margin-left:auto">${total?fmt$0(total)+'/yr':''}</span></div>
      <div class="cbody tight"><table class="tbl">
        <thead><tr><th>Expense</th><th class="num">Per period</th><th class="num">Yearly</th><th>Deduction</th><th></th></tr></thead>
        <tbody>${rows||`<tr><td colspan="5" class="muted">${emptyMsg}</td></tr>`}</tbody>
      </table></div></div>`;
  };
  ...
  const expRow=(e,i)=>{
    ...
    const rcptBtn=e.receiptId
      ?`<button class="btn ghost small" onclick="rcptView('${e.receiptId}','${esc(e.receiptName||'receipt')}')" title="View receipt">📎</button>`
      :`<button class="btn ghost small" onclick="expAttachFile(${i})" title="Attach receipt">📎+</button>`;
    return `<tr>
      ...
      <td>${e.isDeductible?(e.isWorkDevice
        ?`<span class="badge blue">🖥 Work device</span> <button class="btn ghost small" style="padding:1px 6px;height:auto" onclick="devDetail('${e.linkedDeviceId}','')">Schedule</button>`
        :`<span class="badge euc">✓ ${fmt$0(yearlyFromRec(dedAmt,e.recurrence))}/yr deductible</span>${asset?`<br><span class="badge gold" style="font-size:.68rem">${esc(asset.name)}</span>`:''}`
      ):''}</td>
      <td class="rowact">${rcptBtn}
        <button class="btn ghost small" onclick="expAdd(${i})">Edit</button>
        <button class="btn ghost small" onclick="expDuplicate(${i})" title="Duplicate this expense">⊕</button>
        <button class="btn ghost small" onclick="if(!lockedGuard())expDelete(${i})">✕</button>
      </td></tr>`;
  };

  // ── Categorise expenses ──
  // Group general expenses by category
  let generalRows='',generalTotal=0;
  const generalByCat={};
  const generalCatOrder=[];
  let subRows='',subTotal=0;
  let deviceRows='',deviceTotal=0;
  let dedTotal=0;

  exps.forEach((e,i)=>{
    const yearly=yearlyFromRec(num(e.amount),e.recurrence);
    if(e.isDeductible)dedTotal+=yearlyFromRec(expDeductibleAmt(e),e.recurrence);
    if(e.isWorkDevice){
      deviceRows+=expRow(e,i);
      deviceTotal+=yearly;
      ...
      const cat=e.category||'Other';
      if(!generalByCat[cat]){generalByCat[cat]={rows:'',total:0};generalCatOrder.push(cat);}
      generalByCat[cat].rows+=expRow(e,i);
      generalByCat[cat].total+=yearly;
      generalTotal+=yearly;
    }
  });
  // Build grouped rows with a subheader per category
  generalCatOrder.sort().forEach(cat=>{
    const {rows,total}=generalByCat[cat];
    generalRows+=`<tr class="subhead"><td colspan="5" style="padding:6px 16px 4px;background:color-mix(in srgb,var(--surface2) 60%,transparent);font-size:.78rem;font-weight:var(--fw-bold);color:var(--muted);letter-spacing:.04em;text-transform:uppercase">${esc(cat)}<span style="float:right;font-variant-numeric:tabular-nums;font-weight:var(--fw-semibold)">${fmt$0(total)}/yr</span></td></tr>${rows}`;
  });

  // ── Investment property expenses (from Assets) ──
  const propExps=(B.property?.expenses||[]).slice().sort((x,y)=>
    (x.category||'').localeCompare(y.category||'')||((x.date||'')<(y.date||'')?1:-1));
  ...
  propertyAssets.forEach(a=>{
    const fee=managementFeeForFY(a,y);if(!fee)return;
    mgmtFeeTotal+=fee;
    const curRate=managementFeeCurrentRate(a);
    mgmtFeeRows+=`<tr>
      <td><b>Property management fee</b><div class="muted" style="font-size:.74rem">${esc(a.name)}${curRate?` · ${curRate.pct}% of rent`:''}</div></td>
      <td class="num muted">—</td><td class="num">${fmt$0(fee)}</td>
  ...
  const propRows=propExps.map(e=>{
    ...
    return `<tr>
      <td><b>${esc(e.item||e.category)}</b><div class="muted" style="font-size:.74rem">${esc(e.category)} · ${fmtDate(e.date)}</div></td>
      <td class="num muted">—</td><td class="num">${fmt$0(full)}</td>
      <td><span class="badge euc">${ded>=full?'✓ fully deductible':fmt$0(ded)+' deductible'}</span></td>
      <td class="rowact"><a href="#" onclick="go('assets');return false" class="btn ghost small">Edit in Assets</a></td></tr>`;
  }).join('');
  const propTotal=propExpTotal+mgmtFeeTotal;
  const propAllRows=propRows+mgmtFeeRows;

  const yearlyTotal=generalTotal+subTotal+deviceTotal+propTotal;
  ...
  // ── Stat cards ──
  m.insertAdjacentHTML('beforeend',`<div class="grid3" style="margin-bottom:18px">
  ...
  // ── Sections ──
  m.insertAdjacentHTML('beforeend',sectionTable('General expenses','recurring and one-off household costs',generalRows,generalTotal,'No general expenses yet — add your mortgage, rent, groceries, utilities…'));
  if(subRows||true)m.insertAdjacentHTML('beforeend',sectionTable('📱 Subscriptions','recurring services and memberships',subRows,subTotal,'No subscriptions recorded — add Netflix, Spotify, cloud storage etc.'));
  if(deviceRows)m.insertAdjacentHTML('beforeend',sectionTable('🖥 Work devices & equipment','depreciation managed on the Deductions page',deviceRows,deviceTotal,''));
  if(propAllRows)m.insertAdjacentHTML('beforeend',sectionTable('🏠 Investment property','from Assets — edit there to change',propAllRows,propTotal,''));

  if(dedTotalAll)m.insertAdjacentHTML('beforeend',`<div class="note" style="margin-bottom:18px">Deductible expenses flow automatically to Deductions and Investment property sections — no double entry needed. Management fees are auto-calculated from the rental income × fee % set in Assets.</div>`);

  // ── Chart at the bottom ──
  m.insertAdjacentHTML('beforeend',`<div class="card"><div class="chead"><h2>Monthly expenses — ${esc(fyDisplay(y))}</h2><span class="hint">recurring and one-off outgoings by month — includes property expenses and management fees</span></div>
...
function expReceiptScan(){
  ...
  modal('Scan receipt / invoice',`
    ...
    `<button class="btn" data-close onclick="_rcptPendingFile=null;_rcptExtracted=null">Cancel</button><button id="rcptScanApply" class="btn primary" style="display:none" onclick="expReceiptApply()">Use these details</button>`);
}
let _rcptExtracted=null;
let _rcptPendingFile=null; // the File object from the scan, carried through to expSave
async function expReceiptProcess(input){
  const file=input.files[0];if(!file)return;
  _rcptPendingFile=file; // keep the File reference so expSave can attach it
...
function expDuplicate(i){
  if(lockedGuard())return;
  const B=PD();B.expenses=B.expenses||[];
  const src=B.expenses[i];if(!src)return;
  // Deep clone minus the receipt and device link — those belong to the original
  const copy={...src,id:uid(),linkedDeviceId:undefined,receiptId:undefined,receiptName:undefined};
  B.expenses.push(copy);
  save();render();
  // Open edit form on the new copy so the user can rename/adjust before saving
  expAdd(B.expenses.length-1);
}
...
function expAdd(i, prefill, forceMode){
  if(lockedGuard())return;
  const y=FY();
  const B=PD();B.expenses=B.expenses||[];
  const deviceDefaults=forceMode==='device'?{category:'Work',recurrence:'once',date:todayISO(),isWorkDevice:true,isDeductible:true,deductibleMode:'device',deviceWorkUse:100,deviceMethod:'immediate',deviceLife:3}:{};
  const defaults={name:'',category:'Housing',amount:'',recurrence:'monthly',schedDay:'',date:'',isDeductible:false,deductibleMode:'fixed',deductibleAmount:'',deductiblePct:'',propCategory:'Interest',assetId:'',note:'',...deviceDefaults};
  const e=i!=null?B.expenses[i]:(prefill?{...defaults,...prefill}:{...defaults});
  const isDeviceMode=forceMode==='device'||e.isWorkDevice;
  ...
  const pendingFile=prefill&&_rcptPendingFile?_rcptPendingFile:null;
  const rcptSection=pendingFile
    ?`<div class="note mt" style="border-color:var(--euc)">📎 Receipt will be attached: <b>${esc(pendingFile.name)}</b></div>`
    :(i!=null
      ?(e.receiptId
        ?`<div class="mt"><span class="rcpt-file" onclick="rcptView('${e.receiptId}','${esc(e.receiptName||'receipt')}')">📎 ${esc(e.receiptName||'receipt')}</span> <button class="btn ghost small" onclick="expDetachReceipt(${i});closeModal()">Remove</button></div>`
        :`<div class="mt"><label class="fld">Attach receipt</label><input id="f_exp_rcpt_inline" type="file" class="input" accept="image/*,.pdf"></div>`)
      :`<div class="mt"><label class="fld">Attach receipt <span class="muted">(optional)</span></label><input id="f_exp_rcpt_inline" type="file" class="input" accept="image/*,.pdf"></div>`);
  const dedMode=e.deductibleMode||'fixed';
  const title=pendingFile?'Add expense (from receipt)':(i!=null?'Edit expense':'Add expense');
  modal(title,`
    ...
    <hr style="border:none;border-top:1px solid var(--line);margin:14px 0">
    <label style="display:flex;gap:8px;align-items:center;margin-bottom:10px"><input type="checkbox" id="f_ded" ${e.isDeductible||isDeviceMode?'checked':''} onchange="expDedChange(this.checked)"> This expense has a <b>tax-deductible portion</b></label>
    <div id="expDedFlds" style="display:${e.isDeductible||isDeviceMode?'block':'none'}">
      <div class="fldrow">
        <div><label class="fld">Claimable as</label><select id="f_dmode" class="input" onchange="expDedModeChange(this.value)">
          <option value="fixed" ${e.deductibleMode==='fixed'&&!isDeviceMode?'selected':''}>Fixed $ amount per period</option>
          <option value="percent" ${e.deductibleMode==='percent'&&!isDeviceMode?'selected':''}>% of the amount per period</option>
          <option value="device" ${isDeviceMode?'selected':''}>Work device / equipment (multi-year depreciation)</option></select></div>
        <div id="expDedFixedFld" style="display:${e.deductibleMode==='fixed'&&!isDeviceMode?'block':'none'}"><label class="fld">Claimable amount ($ per period)</label><input id="f_da" class="input money" value="${e.deductibleAmount||''}" placeholder="e.g. 900 (interest only)"></div>
        <div id="expDedPctFld" style="display:${e.deductibleMode==='percent'&&!isDeviceMode?'block':'none'}"><label class="fld">Claimable %</label><input id="f_dpct" class="input money" value="${e.deductiblePct||''}" placeholder="e.g. 80" oninput="expDedPctPreview()"></div>
      </div>
      <div class="hint" id="expDedPctHint" style="margin-top:4px"></div>
      <div id="expDeviceFlds" style="display:${isDeviceMode?'block':'none'}">
        ...
        <div class="mt"><label class="fld">Receipt / invoice <span class="muted">(recommended — attach the purchase tax invoice)</span></label>
          ${pendingFile
            ?`<div class="note" style="border-color:var(--euc);margin-top:4px">📎 Receipt will be attached: <b>${esc(pendingFile.name)}</b></div>`
            :e.receiptId
              ?`<div style="display:flex;align-items:center;gap:8px;margin-top:4px"><span class="rcpt-file" onclick="rcptView('${e.receiptId}','${esc(e.receiptName||'receipt')}')">📎 ${esc(e.receiptName||'receipt')}</span> <button class="btn ghost small" onclick="expDetachReceipt(${i??'null'});closeModal()">Remove</button></div>`
              :`<input id="f_exp_rcpt_inline" type="file" class="input mt" accept="image/*,.pdf">`
          }
        </div>
      ...
      <div class="fldrow mt" id="expDedCatRow" style="display:${isDeviceMode?'none':'flex'}"><div style="flex:2"><label class="fld">Deduction category</label><select id="f_dcat" class="input">
      ...
      <div class="fldrow mt" id="expDedAssetRow" style="display:${isDeviceMode?'none':'flex'}"><div style="flex:2"><label class="fld">Linked property asset <span class="muted">(optional — only for property-related deductions)</span></label>
        <select id="f_dasset" class="input"><option value="">— no asset link —</option>${propOpts}</select></div></div>
      <div class="note" id="expDedAssetNote" style="display:${isDeviceMode?'none':'block'}">If linked to a property, the deductible portion flows through to that property's expenses automatically. Otherwise it's included as a general work-related deduction.</div>
    </div>
    ${isDeviceMode?'':rcptSection}`,
...
function expSave(i){
  ...
  const e={
    ...
    // Preserve receipt attachment — not in any form field; the modal shows
    // it read-only. Only cleared if the user explicitly clicks Remove.
    receiptId:i!=null?B.expenses[i].receiptId:undefined,
    receiptName:i!=null?B.expenses[i].receiptName:undefined,
  };
  ...
  // Capture the inline file picker NOW, before closeModal() destroys the DOM.
  const inlineFile=$('#f_exp_rcpt_inline')?.files[0];
  save();closeModal();render();toast('Expense saved');
  const fileToAttach=_rcptPendingFile||inlineFile;
  _rcptPendingFile=null;
  if(fileToAttach){
    const idx=i!=null?i:B.expenses.length-1;
    const savedExp=B.expenses[idx];
    if(savedExp){
      const oldRid=savedExp.receiptId;
      savedExp.receiptId=uid();savedExp.receiptName=fileToAttach.name;
      save();
      rcptPut({id:savedExp.receiptId,name:fileToAttach.name,type:fileToAttach.type,blob:fileToAttach},{fy:fyDisplay(FY()),category:'Expenses',date:savedExp.date||todayISO(),itemName:savedExp.name||'Expense',pid:DB.currentPid})
        .then(()=>{
          if(oldRid&&!receiptStillReferenced(oldRid))rcptDel(oldRid).catch(()=>{});
          toast('Receipt attached');render();
        })
        .catch(()=>toast("Expense saved but couldn't attach receipt — use 📎 to try again"));
    }
  }
}
...
const GD={
  ...
  // Attempt a silent (no popup) token refresh using the browser's existing
  // Google session — succeeds if the user is still signed into Google in
  // this browser, fails if they've signed out or revoked access (in which
  // case the caller should fall back to a full, visible re-connect).
  async silentReauth(){
    const clientId=OAUTH_CLIENT_ID||this.cfg.clientId;
    if(!clientId||!this.cfg.userEmail)throw new Error('Not previously connected');
    await this.loadGIS();
    const tok=await new Promise((res,rej)=>{
      const c=google.accounts.oauth2.initTokenClient({
        client_id:clientId,scope:this.SCOPES,
        callback:t=>{if(t.error)rej(new Error(t.error));else res(t);},
        error_callback:e=>rej(new Error(e?.type||'failed'))
      });
      c.requestAccessToken({prompt:'none',login_hint:this.cfg.userEmail});
    });
    this.token=tok;
  },
  ...
  async connect(){
    ...
    return new Promise((res,rej)=>{
      ...
      //
      // Delay before attaching the listener: opening any new window/tab on
      // mobile causes the current tab to lose then immediately regain focus
      // (OS window-manager behaviour). Without the delay, that launch-focus
      // fires the listener instantly, the 2500ms countdown starts before the
      // user has even seen the sign-in screen, and the promise rejects mid-
      // sign-in. Wait 3s so the launch-focus noise has settled; after that
      // a focus return genuinely means the window closed on its own.
...
--${boundary}--`
  ...
  /* Write to a file by its Drive ID directly — used when accessing a shared file. */
  async writeJSONById(fileId,content){
    const blob=new Blob([content],{type:'application/json'});
    const hdrs={'Authorization':'Bearer '+this.token,'Content-Type':'application/json'};
    const res=await fetch(this.UPLOAD+`/files/${fileId}?uploadType=media&fields=id`,
      {method:'PATCH',headers:hdrs,body:blob});
  ...
  /* Read a file by its Drive ID directly — used when accessing a shared file. */
  async readJSONById(fileId){
    const blob=await this.download(fileId);
    return blob.text();
  },
  /* Resolve the Drive file ID of data.json in the default Ledger folder.
     Useful for sharing: Matt can get this ID and share it with his wife. */
  async getDataFileId(){
    const folderId=await this.ensureFolderPath(['Applications','Ledger']);
    const q=encodeURIComponent(`name='data.json' and '${folderId}' in parents and trashed=false`);
    const r=await this.api('GET',`/files?q=${q}&fields=files(id)`);
    return r.files?.[0]?.id||null;
  },
  /* Share a Drive file with another Google account (give editor/writer access).
     Requires the drive.file scope — which we already have. */
  async shareFileWith(fileId,email,role='writer'){
    const res=await fetch(`https://www.googleapis.com/drive/v3/files/${fileId}/permissions`,{
      method:'POST',
      headers:{'Authorization':'Bearer '+this.token,'Content-Type':'application/json'},
      body:JSON.stringify({type:'user',role,emailAddress:email,sendNotificationEmail:true}),
    });
    if(!res.ok)throw new Error('Could not share: '+(await res.text()));
    return res.json();
  },
...
function collectReceiptIds({fyKey,pid}={}){
  ...
  Object.entries(DB.years).forEach(([k,y])=>{
    ...
    DB.people.forEach(p=>{
      ...
      (b.expenses||[]).forEach(e=>{if(e.receiptId)ids.push(e.receiptId);});
  ...
  if(fyKey==null&&pid==null){
    ...
    (DB.assets||[]).forEach(a=>{
      ...
      if(a.depReportId)ids.push(a.depReportId);
    });
  }
  return ids;
}
...
let _rcptIdSetCache=null;
function receiptStillReferenced(receiptId){
  if(!receiptId)return false;
  if(!_rcptIdSetCache){
    _rcptIdSetCache=new Set(collectReceiptIds());
    Promise.resolve().then(()=>{_rcptIdSetCache=null;});
  }
  return _rcptIdSetCache.has(receiptId);
}
...
async function rcptView(id,name){
  // Open the tab FIRST, synchronously, while the browser still considers
  // this a direct user gesture — window.open() after an await gets blocked.
  const w=window.open('','_blank');
  if(!w){
    // Blocked — fetch and trigger a download as fallback
    const rec=await rcptGet(id);
    if(!rec)return toast('Receipt not found');
    const mime=rec.type||'application/octet-stream';
    const blob=rec.blob instanceof Blob?rec.blob:new Blob([rec.blob||''],{type:mime});
    const url=URL.createObjectURL(blob);
    const a=document.createElement('a');a.href=url;a.download=rec.name||name||'receipt';a.click();
    setTimeout(()=>URL.revokeObjectURL(url),5000);
    toast('Popup blocked — downloading instead. Allow popups for this site to open files in a tab.');
    return;
  }
  // Show a loading page immediately while we fetch
  w.document.write('<!DOCTYPE html><html><head><meta charset="utf-8"><title>Loading…</title><style>body{margin:0;display:flex;align-items:center;justify-content:center;height:100vh;font-family:system-ui,sans-serif;color:#666;background:#f5f5f5}</style></head><body>Loading…</body></html>');
  w.document.close();
  // Async fetch — the window is already open so this is fine
  const rec=await rcptGet(id);
  if(!rec){
    w.document.open();
    w.document.write('<!DOCTYPE html><html><body style="font-family:system-ui;padding:40px;color:#c00">Receipt not found in this browser.</body></html>');
    w.document.close();
    return;
  }
  const mime=rec.type||'application/octet-stream';
  const rawBlob=rec.blob instanceof Blob?(rec.blob.type?rec.blob:new Blob([rec.blob],{type:mime})):new Blob([rec.blob||''],{type:mime});
  const filename=rec.name||name||'file';
  const url=URL.createObjectURL(rawBlob);
  const esc2=s=>String(s).replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;');
  const isImg=mime.startsWith('image/');
  // Both images and PDFs get a toolbar with a named download link — the only
  // way to guarantee the right filename regardless of what the browser's
  // native viewer infers from the blob URL.
  const toolbar=`<div style="position:fixed;top:0;left:0;right:0;height:40px;background:rgba(0,0,0,.75);display:flex;align-items:center;padding:0 14px;gap:12px;z-index:9999;font:13px system-ui">
    <span style="color:#ccc;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc2(filename)}</span>
    <a href="${esc2(url)}" download="${esc2(filename)}" style="color:#fff;text-decoration:none;background:rgba(255,255,255,.15);border:1px solid rgba(255,255,255,.3);padding:4px 12px;border-radius:5px;white-space:nowrap">⬇ Save ${esc2(filename)}</a>
  </div>`;
  w.document.open();
  if(isImg){
    w.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>${esc2(filename)}</title>
    <style>*{margin:0;padding:0;box-sizing:border-box}body{background:#111;padding-top:40px;display:flex;align-items:center;justify-content:center;min-height:100vh}
    img{max-width:100vw;max-height:calc(100vh - 40px);object-fit:contain}</style></head>
    <body>${toolbar}<img src="${esc2(url)}" alt="${esc2(filename)}"></body></html>`);
  }else{
    w.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>${esc2(filename)}</title>
    <style>*{margin:0;padding:0}html,body{height:100%}iframe{display:block;width:100%;height:calc(100% - 40px);margin-top:40px;border:none}</style></head>
    <body>${toolbar}<iframe src="${esc2(url)}" title="${esc2(filename)}"></iframe></body></html>`);
  }
  w.document.close();
...
function propAdd(i){
  ...
  const e=i!=null?B.property.expenses[i]:{date:todayISO(),category:cats[0],item:'',amount:'',recurrence:'once',schedDay:''};
  ...
  const isRecurring=rec!=='once';
  modal(i!=null?'Edit expense':(isVehicle?'Add vehicle expense':'Add property expense'),`
    <div class="fldrow">
      <div><label class="fld">${isRecurring?'Start date':'Date'}</label><input id="f_d" type="date" class="input" value="${e.date}"></div>
      <div><label class="fld">Category</label><select id="f_c" class="input">${cats.map(c=>`<option ${e.category===c?'selected':''}>${c}</option>`).join('')}</select></div>
    </div>
    <div class="fldrow mt">
      <div><label class="fld">Frequency</label><select id="f_prec" class="input" onchange="propRecChange(this.value)">
        <option value="once" ${rec==='once'?'selected':''}>One-off</option>
      ...
      <div id="f_pday_row" style="display:${isRecurring?'block':'none'}"><label class="fld">Day of month</label><input id="f_pday" class="input money" value="${e.schedDay||''}" placeholder="e.g. 1"></div>
    </div>
    <div class="fldrow mt"><div style="flex:2"><label class="fld">Detail</label><input id="f_i" class="input" value="${esc(e.item)}" placeholder="${isVehicle?'e.g. Annual rego, comprehensive insurance':'e.g. Bank account fee, council rates'}"></div>
    <div><label class="fld" id="f_a_label">Amount ($)</label><input id="f_a" class="input money" value="${e.amount}" oninput="propAmountChange()"></div></div>
    ...
    <div class="hint">Defaults to the full amount — most expenses like insurance, rates and fees are fully deductible. For a mortgage, enter the full repayment as Amount and only the <b>interest portion</b> here.${isVehicle?' For a vehicle, this is typically the logbook work-use %.':''}</div>`}
...
function propRecChange(v){
  const dayRow=$('#f_pday_row');if(dayRow)dayRow.style.display=v!=='once'?'block':'none';
  const dateLabel=$('#f_d')?.previousElementSibling;
  if(dateLabel)dateLabel.textContent=v!=='once'?'Start date':'Date';
}
...
async function propSave(i){
  ...
  e.date=$('#f_d').value;e.category=$('#f_c').value;e.item=$('#f_i').value.trim();
  e.amount=num($('#f_a').value);
  e.recurrence=$('#f_prec')?.value||'once';
  e.schedDay=e.recurrence!=='once'?($('#f_pday')?.value||''):'';
  const daField=$('#f_da');
...
function devDetail(devId, purchasedInLabel){
  ...
  // The current FY for display — use FY() which always returns the best
  // usable year regardless of whether DB.currentFY is 'all' or a key
  const currentFY=FY();
  const currentOrder=fyOrderYear(currentFY);
  const purchaseOrder=fyOrderYear(purchaseFY);

  // Build a schedule that covers the FULL depreciation life, even if those
  // future FYs haven't been created yet.  We first collect existing FYs,
  // then pad forward with synthetic placeholders until the asset is fully
  // depreciated (or immediate write-off is exhausted).
  const existingFYs=Object.values(DB.years).sort((a,b)=>fyOrderYear(a)-fyOrderYear(b));
  const lastExistingOrder=existingFYs.length?fyOrderYear(existingFYs[existingFYs.length-1]):purchaseOrder;

  // Helper: compute deduction for a given year expressed as a calendar-order
  // integer offset from the purchase year.  We reimplement the core of
  // deviceDeductionForFY here so we can run it over synthetic future years.
  function claimForOffset(offset){
    if(dev.method==='immediate')return offset===0?workAmt:0;
    const life=dev.life||3;
    const purchaseDate=new Date(dev.date+'T00:00:00Z');
    const {start:pStart,end:pEnd}=fyRange(purchaseFY);
    const fyStart=new Date(pStart+'T00:00:00Z');
    const fyEnd=new Date(pEnd+'T00:00:00Z');
    // Advance by offset years (approximate — one FY = 365 days)
    const targetStart=new Date(fyStart);targetStart.setFullYear(targetStart.getFullYear()+offset);
    const targetEnd=new Date(fyEnd);targetEnd.setFullYear(targetEnd.getFullYear()+offset);
    const isPurchaseYear=offset===0;
    const daysInTargetYear=(targetEnd-targetStart)/86400000+1;
    const purchase=purchaseDate.getTime();
    const days=isPurchaseYear?Math.max(1,Math.round((targetEnd.getTime()-Math.max(purchase,targetStart.getTime()))/86400000)+1):daysInTargetYear;

    let base=workAmt,totalClaimed=0;
    for(let o=0;o<offset;o++){
      const prevStart=new Date(fyStart);prevStart.setFullYear(prevStart.getFullYear()+o);
      const prevEnd=new Date(fyEnd);prevEnd.setFullYear(prevEnd.getFullYear()+o);
      const prevDays=o===0?Math.max(1,Math.round((prevEnd.getTime()-Math.max(purchase,prevStart.getTime()))/86400000)+1):(prevEnd-prevStart)/86400000+1;
      let prevDec;
      if(dev.method==='diminishing')prevDec=base*(2/life)*(prevDays/((prevEnd-prevStart)/86400000+1));
      else prevDec=workAmt*(1/life)*(prevDays/((prevEnd-prevStart)/86400000+1));
      prevDec=Math.min(prevDec,dev.method==='diminishing'?base:workAmt-totalClaimed);
      base-=Math.max(0,prevDec);totalClaimed+=Math.max(0,prevDec);
    }
    let dec;
    if(dev.method==='diminishing')dec=base*(2/life)*(days/daysInTargetYear);
    else dec=workAmt*(1/life)*(days/daysInTargetYear);
    dec=Math.min(dec,dev.method==='diminishing'?base:workAmt-totalClaimed);
    return Math.max(0,dec);
  }

  // Determine how many years forward to show: enough to cover full life
  // (prime/diminishing converge to 0 after `life` years; we show up to life+1
  // to confirm full depreciation) or until no meaningful claim remains.
  const maxOffset=dev.method==='immediate'?0:Math.ceil((dev.life||3)*1.5)+1;
  const schedule=[];
  let runningWDV=workAmt;
  for(let offset=0;offset<=maxOffset;offset++){
    const claim=claimForOffset(offset);
    if(claim<0.005&&offset>0&&dev.method!=='immediate')break; // fully depreciated
    // Find the actual FY object for this offset if it exists
    const targetOrder=purchaseOrder+offset;
    const realFY=existingFYs.find(fy=>fyOrderYear(fy)===targetOrder)||null;
    const label=realFY?fyDisplay(realFY):`FY${String(purchaseFY.startYear+offset).slice(-2)}–${String(purchaseFY.startYear+offset+1).slice(-2)}`;
    const openingWDV=dev.method!=='immediate'?runningWDV:null;
    // Status: past / current / upcoming — based on fyOrderYear vs now
    const status=targetOrder<currentOrder?'past':targetOrder===currentOrder?'current':'upcoming';
    const isCurrent=targetOrder===currentOrder;
    const isLocked=realFY?.locked||false;
    schedule.push({offset,claim,openingWDV,label,isCurrent,status,isLocked,realFY});
    runningWDV=Math.max(0,runningWDV-claim);
  }

  const claimedSoFar=schedule.filter(r=>r.status==='past'||r.status==='current').reduce((s,r)=>s+r.claim,0);
  const fullLifeClaim=schedule.reduce((s,r)=>s+r.claim,0);
  const remainingFuture=Math.max(0,fullLifeClaim-claimedSoFar);

  const pctClaimed=workAmt>0?Math.min(100,claimedSoFar/workAmt*100):0;
  const progressBar=`<div style="background:var(--surface2);border-radius:6px;height:8px;margin-top:8px;overflow:hidden">
    ...
    <div style="font-size:.78rem;color:var(--muted);margin-top:4px">${fmt$(claimedSoFar)} claimed so far of ${fmt$(workAmt)} (${pctClaimed.toFixed(0)}%)${remainingFuture>0.01?' · '+fmt$(remainingFuture)+' remaining over life':' · fully claimed'}</div>`;

  const fyRows=schedule.map(r=>`<tr ${r.isCurrent?'style="font-weight:700;background:var(--euc-soft)"':''}>
    <td>${esc(r.label)}${r.isCurrent?' <span class="badge euc" style="font-size:.65rem">current</span>':''}${!r.realFY?' <span class="badge" style="font-size:.65rem;color:var(--muted)">not created yet</span>':''}</td>
    ${dev.method!=='immediate'?`<td class="num">${r.openingWDV!=null?fmt$(r.openingWDV):'—'}</td>`:''}
    <td class="num" style="color:var(--euc)">${r.claim>0?fmt$(r.claim):'<span class="muted">—</span>'}</td>
    <td>${r.isLocked?'🔒 frozen':r.status==='past'?'<span class="muted">past</span>':r.status==='current'?'<span style="color:var(--euc)">claimable</span>':'<span class="muted">upcoming</span>'}</td></tr>`).join('');

  // Find the device's index in its purchase FY bucket for the Edit button
  const purchaseBucket=bucket(purchaseFY,pid);
  const devIdx=purchaseBucket.devices.findIndex(x=>x.id===devId);
  const editBtn=dev.linkedExpenseId
    ?`<button class="btn" onclick="closeModal();expenseGoToEdit('${dev.linkedExpenseId}')">Edit in Expenses</button>`
    :`<button class="btn" onclick="devAdd(${devIdx})">Edit device</button>`;
  ...
  modal(`${esc(dev.name)} — depreciation schedule`,`
    ...
    <tbody>${fyRows}
      <tr class="total"><td colspan="${dev.method!=='immediate'?2:1}">Total over life</td>
        <td class="num">${fmt$(fullLifeClaim)}</td><td></td></tr>
    </tbody></table>
    ${dev.method!=='immediate'?'<div class="note">Future years marked "not created yet" will claim automatically once you add those financial years in Settings. Opening value = prior year value minus prior year deduction.</div>':''}
    ${dev.method==='immediate'?'<div class="note">Immediate write-off: the full work-related cost is claimed in the purchase year in one go.</div>':''}`,
    `${editBtn}<button class="btn primary" data-close>Close</button>`);
}
...
/* Find all devices that were added directly (not via Expenses) — these
   have no linkedExpenseId. Used by the migration prompt. */
function findOrphanDevices(){
  const orphans=[];
  Object.values(DB.years).forEach(fy=>{
    Object.entries(fy.people||{}).forEach(([pid,b])=>{
      (b.devices||[]).forEach((d,i)=>{
        if(!d.linkedExpenseId)orphans.push({d,b,pid,fy,i});
      });
    });
  });
  return orphans;
}
/* One-time migration: creates an Expense entry for every orphaned device
   and links them bidirectionally. Shows a toast with the count. */
function migrateDevicesToExpenses(){
  const orphans=findOrphanDevices();
  if(!orphans.length){toast('No devices to migrate — all already linked to Expenses');return;}
  let count=0;
  orphans.forEach(({d,b,pid,fy})=>{
    const exp={
      id:uid(),
      name:d.name,
      category:'Work',
      amount:d.cost,
      recurrence:'once',
      date:d.date||fyRange(fy).start,
      isDeductible:true,
      isWorkDevice:true,
      deviceWorkUse:d.workUse||100,
      deviceMethod:d.method||'immediate',
      deviceLife:d.life||3,
      linkedDeviceId:d.id,
      schedDay:'',note:'',
      deductibleMode:'device',
      propCategory:'',assetId:'',
      deductibleAmount:'',deductiblePct:'',
      dedPid:pid
    };
    b.expenses=b.expenses||[];
    b.expenses.push(exp);
    d.linkedExpenseId=exp.id;
    count++;
  });
  save();render();
  toast(`Migrated ${count} device${count===1?'':'s'} to Expenses — edit and attach receipts there`);
}
...
PAGES.other=m=>{
  ...
  head(m,'Deductions',`Everything you can claim for ${esc(fyDisplay(y))} — work-from-home, devices, donations, and other work-related expenses, all in one place.`,
    `<button class="btn" onclick="go('expenses');setTimeout(()=>expAdd(null,null,'device'),150)">+ Add device</button>
  ...
  if(propAssets.length){
    ...
    lineRows.sort((x,y2)=>(x.type||'').localeCompare(y2.type||'')||((x.date||'')<(y2.date||'')?1:-1));
  ...
  // ---- one-time migration prompt for devices added before the Expenses flow ----
  const orphans=findOrphanDevices().filter(({b,pid:opid})=>
    // Only show for the current person (or all people if isAll)
    isAll()||(opid===pid0)
  );
  if(orphans.length){
    m.insertAdjacentHTML('beforeend',`<div class="card" style="border-color:var(--gold-badge);margin-bottom:18px">
      <div class="chead" style="background:color-mix(in srgb,var(--gold-soft) 40%,var(--surface))">
        <h2 style="color:var(--gold-badge)">🔄 Work devices need migrating</h2>
      </div>
      <div class="cbody">
        <p style="font-size:.88rem;color:var(--muted);margin-bottom:12px">
          ${orphans.length} device${orphans.length===1?'':'s'} (${orphans.map(o=>esc(o.d.name)).join(', ')}) ${orphans.length===1?'was':'were'} added directly to Deductions before we moved device tracking to Expenses.
          Migrating creates a matching Expenses entry for each one so you can attach receipts, edit from Expenses, and see them in the full expense history.
          <b>Your depreciation schedules and deduction amounts are unchanged.</b>
        </p>
        <div style="display:flex;gap:10px;flex-wrap:wrap">
          <button class="btn primary" onclick="migrateDevicesToExpenses()">Migrate ${orphans.length} device${orphans.length===1?'':'s'} now</button>
          <button class="btn ghost" onclick="this.closest('.card').remove()">Dismiss</button>
        </div>
      </div>
    </div>`);
  }
  ...
  Object.values(DB.years).sort((a,b)=>a.startYear-b.startYear).forEach(fy=>{
    bucket(fy,pid).devices.forEach((d,i)=>{
      ...
      if(ded>0||owned)drows.push({d,ded,owned,i,fyLabel:fy.label,fyStart:fy.startYear});
    });
  });
  const dtot=drows.reduce((s,r)=>s+r.ded,0);
  const deviceRows=drows.map(r=>{
    // Find the linked expense (if any) to show receipt and allow editing there
    const linkedExp=r.d.linkedExpenseId?(B.expenses||[]).find(e=>e.id===r.d.linkedExpenseId):null;
    const rcptCell=linkedExp?.receiptId
      ?`<span class="rcpt-file" onclick="rcptView('${linkedExp.receiptId}','${esc(linkedExp.receiptName||r.d.name)}')" title="View receipt">📎</span>`
      :`<span class="muted">—</span>`;
    const editBtn=r.owned
      ?`<button class="btn ghost small" onclick="${linkedExp
          ?`expenseGoToEdit('${r.d.linkedExpenseId}')`
          :r.d.linkedExpenseId
            ?`expenseGoToEdit('${r.d.linkedExpenseId}')`
            :`go('expenses');setTimeout(()=>{toast('This device was added directly — use Add device to add new ones through Expenses')},100)`
        }">Edit</button>`
      :'';
    const delBtn=r.owned
      ?`<button class="btn ghost small" onclick="${linkedExp
          ?`devDeleteLinked('${r.d.linkedExpenseId}')`
          :`if(!lockedGuard())confirmDel('Delete ${esc(r.d.name)}?',()=>{PD().devices.splice(${r.i},1);save();render()})`
        }">✕</button>`
      :'';
    return `<tr>
      <td><b>${esc(r.d.name)}</b>
        ${r.owned?'':`<div class="muted" style="font-size:.72rem">purchased in ${esc(r.fyLabel)}</div>`}
        ${linkedExp?`<div class="muted" style="font-size:.72rem">🧾 from <a href="#" onclick="expenseGoToEdit('${r.d.linkedExpenseId}')">Expenses</a></div>`:`<div class="muted" style="font-size:.72rem">added directly — <a href="#" onclick="go('expenses');setTimeout(()=>expAdd(null,null,'device'),150)">add future items via Expenses</a></div>`}</td>
      <td>${fmtDate(r.d.date)}</td>
      <td class="num">${fmt$(r.d.cost)}</td>
      <td class="num">${r.d.workUse||100}%</td>
      ...
      <td class="num">${rcptCell}</td>
      <td class="num"><b>${fmt$(r.ded)}</b></td>
      <td class="rowact">${editBtn}${delBtn}</td></tr>`;
  }).join('');
  m.insertAdjacentHTML('beforeend',`
  <div class="card"><div class="chead"><h2>Work devices & equipment</h2>
    <span class="hint">Record purchases in <a href="#" onclick="go('expenses');setTimeout(()=>expAdd(null,null,'device'),150)">Expenses</a> — depreciation flows here automatically</span></div>
  <div class="cbody tight"><table class="tbl">
    <thead><tr><th>Item</th><th>Purchased</th><th class="num">Cost</th><th class="num">Work use</th><th>Method</th><th>Receipt</th><th class="num">Claim in ${esc(y.label)}</th><th></th></tr></thead>
    <tbody>${deviceRows||'<tr><td colspan="8" class="muted">No devices yet — use <a href="#" onclick="go(\'expenses\');setTimeout(()=>expAdd(null,null,\'device\'),150)">+ Add device</a> above.</td></tr>'}
    ${dtot?`<tr class="total"><td colspan="6">Device deductions claimable in ${esc(y.label)}</td><td class="num">${fmt$(dtot)}</td><td></td></tr>`:''}</tbody></table></div></div>
  <div class="note">Record each device purchase in <a href="#" onclick="go('expenses');setTimeout(()=>expAdd(null,null,'device'),150)">Expenses</a> — choose "Work device / equipment" as the deductible type. Attach a receipt or invoice there. Items ≤ ${fmt$0(cap)} work portion claim immediately; dearer items depreciate over their effective life and carry forward automatically.</div>`);
};
...
function scheduleDriveSync(){
  // Schedule even if the token is currently expired, as long as we have
  // stored credentials to silently refresh it with — otherwise a save()
  // that happens to land while the token is stale would skip sync
  // entirely until the next manual click.
  if(!GD.isConnected()&&!GD.cfg.userEmail)return;
...
async function pushToDrive(){
  if(!GD.isConnected()){
    // Token expired but we were connected before — try a silent (no
    // popup) refresh using the browser's existing Google session before
    // giving up, so sync (manual click or background) "just works" again
    // without the user having to manually reconnect via Settings.
    if(!GD.cfg.userEmail)return;
    try{await GD.silentReauth();}
    catch(e){setSyncStatus('error','⚠ Session expired — click Sync to reconnect');return;}
  }
  try{
    ...
    if(payload.rcptMeta)Object.values(payload.rcptMeta).forEach(m=>{delete m.blob;});
    const sharedId=DB.gdrive?.sharedFileId;
    if(sharedId){
      await GD.writeJSONById(sharedId,JSON.stringify(payload));
    }else{
...
async function pullFromDrive(silent){
  ...
  try{
    const sharedId=DB.gdrive?.sharedFileId;
    const text=sharedId?await GD.readJSONById(sharedId):await GD.readJSON(SYNC_FILE);
    if(!text)return null;
...
function cgtBreakdown(b){
  ...
  // return where prior-year losses fully absorbed a small gross fund gain
  // (18H), correctly reducing net capital gain (18A-equivalent) to exactly
  // $0, with the remainder carried forward matching prior losses minus the
  // GROSS 18H figure exactly. Applying losses to the post-discount 18A
  // instead would only consume half as much loss balance for the same
  // gain, which doesn't match how the ATO actually calculates it. AMIT
  // funds report 18A net of their own internal 50% discount
...
/* Prorated yearly amount for a pre-tax deduction within a given FY.
   Uses r.startDate if set (any type), leaseStartDate as fallback for
   novated leases. Returns r.yearly unchanged when no start date applies
   or when the deduction was fully active for the whole FY. */
function preTaxDedYearlyFor(r,y){
  const base=num(r.yearly);
  if(r.recurrence==='once')return base;
  if(r.recurrence==='yearly'){
    const startDate=r.startDate||(r.type==='Novated lease'?r.leaseStartDate:'')||r.date||'';
    if(!startDate||!y)return base;
    const{start:fyStart,end:fyEnd}=fyRange(y);
    if(startDate>fyEnd)return 0;
    return base;
  }
  const startDate=r.startDate||(r.type==='Novated lease'?r.leaseStartDate:'')||'';
  if(!startDate||!y)return base;
  const{start:fyStart,end:fyEnd}=fyRange(y);
  if(startDate>fyEnd)return 0;
  if(startDate<=fyStart)return base;

  const perPeriodAmt=base/(REC_MULT[r.recurrence]||12);
  const startD=new Date(startDate+'T00:00:00Z');
  const endD=new Date(fyEnd+'T00:00:00Z');

  if(r.recurrence==='monthly'||r.recurrence==='quarterly'){
    const monthsPerPeriod=r.recurrence==='quarterly'?3:1;
    // If linked to an income source with a specific pay day, inherit it so the
    // deduction fires on the same day as the paycheck (salary sacrifice is
    // deducted at the same time as the salary disbursement).
    let chargeDay=null;
    if(r.incomeId){
      const linkedInc=(()=>{
        for(const fy of Object.values(DB.years)){
          for(const pid of Object.keys(fy.people||{})){
            const inc=(fy.people[pid].incomes||[]).find(x=>x.id===r.incomeId);
            if(inc)return inc;
          }
        }
        return null;
      })();
      if(linkedInc&&num(linkedInc.schedDay)>0)chargeDay=num(linkedInc.schedDay);
    }
    // Count how many times the charge falls between startDate and fyEnd.
    if(chargeDay!=null){
      // Schedday-based: first charge is on chargeDay in start month if chargeDay >= startDate.day,
      // otherwise first charge is in the following period.
      let yr=startD.getUTCFullYear(),mo=startD.getUTCMonth();
      if(chargeDay<startD.getUTCDate())mo+=monthsPerPeriod;
      let periods=0;
      while(true){
        const daysInMonth=new Date(Date.UTC(yr,mo+1,0)).getUTCDate();
        const fireDay=Math.min(chargeDay,daysInMonth);
        const fireDate=new Date(Date.UTC(yr,mo,fireDay));
        if(fireDate>endD)break;
        periods++;
        mo+=monthsPerPeriod;
        if(mo>=12){yr+=Math.floor(mo/12);mo=mo%12;}
      }
      return round2(Math.max(0,periods)*perPeriodAmt);
    }else{
      // No linked income pay day — count calendar months from start month (inclusive).
      const totalMonths=(endD.getUTCFullYear()-startD.getUTCFullYear())*12+(endD.getUTCMonth()-startD.getUTCMonth())+1;
      const periods=r.recurrence==='quarterly'?Math.floor(totalMonths/3):totalMonths;
      return round2(Math.max(0,periods)*perPeriodAmt);
    }
  }else if(r.recurrence==='fortnightly'){
    const activeDays=Math.max(0,Math.floor((endD-startD)/86400000)+1);
    return round2(Math.floor(activeDays/14)*perPeriodAmt);
  }else if(r.recurrence==='weekly'){
    const activeDays=Math.max(0,Math.floor((endD-startD)/86400000)+1);
    return round2(Math.floor(activeDays/7)*perPeriodAmt);
  }else{
    const totalDays=(endD-new Date(fyStart+'T00:00:00Z'))/86400000+1;
    const activeDays=Math.max(0,(endD-startD)/86400000+1);
    return activeDays<=0?0:round2(base*activeDays/totalDays);
  }
}
function fyTaxableBreakdown(y,pid){
  ...
  // Pre-tax deductions: exclude any linked to an income source that has
  // alreadyNetOfSacrifice=true — those are already baked into the income figure.
  const netIncomeIds=new Set((b.incomes||[]).filter(r=>r.alreadyNetOfSacrifice).map(r=>r.id));
  const preTaxTotal=(b.preTaxDeds||[]).filter(r=>!r.incomeId||!netIncomeIds.has(r.incomeId)).reduce((s,r)=>s+preTaxDedYearlyFor(r,y),0);
...
function fySummaryNumbers(y,pid){
  ...
  // PHI rebate — government subsidy on private hospital cover premiums, means-tested
  let phiRebate=0;
  if(o.hasPHI&&o.phiPremium>0&&y.rates.phi){
    const phiIncome=bd.taxable; // simplified: use taxable income as proxy for MLS income
    const tier=y.rates.phi.tiers.find(t=>t.maxIncome==null||phiIncome<=t.maxIncome)||y.rates.phi.tiers[y.rates.phi.tiers.length-1];
    const ageAdj=(y.rates.phi.ageAdj||{})[o.phiAge]||0;
    const rebatePct=Math.max(0,tier.rebate+ageAdj);
    phiRebate=num(o.phiPremium)*(rebatePct/100);
  }
  const balance=bd.withheld+paygInstalments+offsets+phiRebate-tax.total; // + = refund estimate
  const effRate=assessable>0?tax.total/assessable*100:0;
  return{...bd,tax,offsets,phiRebate,balance,paygInstalments,effRate,mlsRate};
}
...
async function mytaxFiledSave(){
  ...
  // Follow-up: prompt to lock this FY and/or start the next one
  const y=FY();
  const nextStartYear=y.startYear+1;
  const nextKey=Object.keys(DB.years).find(k=>DB.years[k].startYear===nextStartYear);
  const alreadyLocked=y.locked;
  const nextExists=!!nextKey;
  if(alreadyLocked&&nextExists)return; // nothing to prompt
  const lockBtn=alreadyLocked?'':`<button class="btn primary" onclick="
    FY().locked=true;save();render();
    this.closest('.modal').querySelector('#postFileLockDone').style.display='';
    this.style.display='none';
    toast('${esc(y.label)} locked — snapshot frozen');
  ">🔒 Lock ${esc(y.label)}</button>
  <div id="postFileLockDone" style="display:none;color:var(--euc);font-size:.86rem;margin-top:4px">✓ Locked</div>`;
  const nextLabel=`FY${String(nextStartYear).slice(2)}–${String(nextStartYear+1).slice(2)}`;
  const nextBtn=nextExists?'':`<button class="btn ${alreadyLocked?'primary':''}" onclick="
    const rates=Object.values(DB.years).sort((a,b)=>b.startYear-a.startYear)[0].rates;
    const fy=newFY(${nextStartYear},rates);
    DB.years[fy.label]=fy;
    DB.currentFY=fy.label;
    save();closeModal();go('dashboard');
    toast('${esc(nextLabel)} created — update rates in Settings if needed');
  ">📅 Start ${esc(nextLabel)}</button>`;
  setTimeout(()=>{
    modal(`${esc(y.label)} filed — what's next?`,`
      <p style="font-size:.88rem;color:var(--muted);margin-bottom:14px">Your return is lodged. A couple of housekeeping steps:</p>
      <div style="display:flex;flex-direction:column;gap:10px">
        ${alreadyLocked?'<div style="color:var(--euc);font-size:.86rem">✓ '+esc(y.label)+' is already locked</div>':
          `<div style="border:1px solid var(--line);border-radius:8px;padding:12px 14px">
            <div style="font-weight:var(--fw-semibold);margin-bottom:4px">Lock ${esc(y.label)}</div>
            <div style="font-size:.83rem;color:var(--muted);margin-bottom:10px">Freezes the year so past figures can't accidentally change. Recommended once your return is accepted.</div>
            ${lockBtn}
          </div>`}
        ${nextExists?'<div style="color:var(--euc);font-size:.86rem">✓ '+esc(nextLabel)+' already exists</div>':
          `<div style="border:1px solid var(--line);border-radius:8px;padding:12px 14px">
            <div style="font-weight:var(--fw-semibold);margin-bottom:4px">Create ${esc(nextLabel)}</div>
            <div style="font-size:.83rem;color:var(--muted);margin-bottom:10px">Start a fresh year for your next return. Rate snapshots carry forward — update them in Settings once the ATO publishes new rates.</div>
            ${nextBtn}
          </div>`}
      </div>`,
      `<button class="btn" data-close>Done</button>`);
  },400); // short delay so the "Marked as filed" toast is visible first
}
...
const DOC_CATS=[
  ...
  {key:'expenses',     label:'Expense receipts',             icon:'🧾', page:'expenses'},
...
function docsCollect(years,pids){
  ...
  years.forEach(y=>{
    ...
    pids.forEach(pid=>{
      ...
      // General expenses — show all deductible ones (whether or not they have a receipt),
      // so the user can see which tax-claimed expenses still need a receipt attached.
      // Non-deductible expenses are silently omitted — the ATO doesn't require receipts for those.
      (b.expenses||[]).forEach(e=>{
        const isDeductible=e.isDeductible||e.isWorkDevice;
        if(!e.receiptId&&!isDeductible)return; // skip non-deductible without receipt
        const desc=esc(e.name||'Expense')+(e.date?` · ${fmtDate(e.date)}`:'');
        const entry={desc,date:e.date||'',pid,fyLabel};
        const cat=cats.get('expenses');
        if(e.receiptId){
          if(!cat.docs.has(e.receiptId))cat.docs.set(e.receiptId,{id:e.receiptId,name:e.receiptName||'Receipt',items:[]});
          cat.docs.get(e.receiptId).items.push(entry);
        }else{
          cat.missing.push(entry);
        }
      });
...
PAGES.trends=m=>{
  ...
  m.insertAdjacentHTML('beforeend',`
    ...
    <div class="grid2" style="gap:14px">
      ${trendCard('Total income','assessable income — salary, dividends, rent, capital gains & franking credits',rows.map(r=>({...r,value:r.totalIncome})),'var(--euc)')}
...
function shareReport(){
  const y=FY(),pid=DB.currentPid,b=PD();
  const s=fySummaryNumbers(y,pid);
  const person=DB.people.find(p=>p.id===pid);
  const personName=person?.name||'Me';
  const now=new Date().toLocaleDateString('en-AU',{day:'numeric',month:'long',year:'numeric'});
  const fy=fyDisplay(y);
  const fmt=v=>`$${Math.round(v).toLocaleString('en-AU')}`;

  // Income breakdown
  const incRows=(b.incomes||[]).map(r=>`<tr><td>${esc(r.name)}</td><td>${esc(r.kind||'')}</td><td class="num">${fmt(r.yearly)}</td></tr>`).join('');
  const divTotal=(b.dividends||[]).reduce((s,d)=>s+num(d.payment),0);
  const fundTotal=(b.funds||[]).reduce((s,f)=>s+Object.values(f.labels||{}).reduce((t,v)=>t+num(v),0),0);

  // Pre-tax deductions
  const ptdRows=(b.preTaxDeds||[]).map(r=>`<tr><td>${esc(r.name)}</td><td>${esc(r.type||'')}</td><td class="num">${fmt(preTaxDedYearlyFor(r,y))}</td></tr>`).join('');

  // Deductions
  const dedBreakdown=s.dedBreakdown||{};
  const dedRows=Object.entries(dedBreakdown).filter(([,v])=>v>0).map(([k,v])=>`<tr><td>${esc(k)}</td><td class="num">${fmt(v)}</td></tr>`).join('');

  // Net worth
  const nwItems=(DB.networth||[]).filter(x=>!x.archived).slice(0,8);

  const css=`body{font-family:system-ui,-apple-system,sans-serif;color:#1a1a1a;max-width:900px;margin:0 auto;padding:32px 24px;font-size:14px;line-height:1.5}
...
th{text-align:left;padding:6px 10px;background:#f3f4f6;font-weight:600;border-bottom:2px solid #d1d5db}
td{padding:6px 10px;border-bottom:1px solid #f0f0f0}
.num{text-align:right;font-variant-numeric:tabular-nums}
.stat-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:24px}
.stat{background:#f9fafb;border:1px solid #e5e7eb;border-radius:8px;padding:14px}
.stat .l{font-size:11px;text-transform:uppercase;letter-spacing:.05em;color:#6b7280;margin-bottom:4px}
.stat .v{font-size:20px;font-weight:700}
.stat.good .v{color:#16a34a}.stat.bad .v{color:#dc2626}.stat.gold .v{color:#d97706}
.total td{font-weight:700;border-top:2px solid #d1d5db;border-bottom:none}
.disclaimer{background:#fef9c3;border:1px solid #fde047;border-radius:8px;padding:14px;font-size:12px;color:#713f12;margin-top:28px}
@media print{.no-print{display:none}.page-break{page-break-before:always}}`;

  const html=`<!DOCTYPE html><html lang="en"><head><meta charset="utf-8">
<title>Ledger — ${esc(personName)} — ${esc(fy)}</title>
<style>${css}</style></head><body>
<div class="no-print" style="margin-bottom:20px;display:flex;gap:10px">
  <button onclick="window.print()" style="padding:8px 16px;background:#1a5e3a;color:#fff;border:none;border-radius:6px;cursor:pointer;font-size:13px">🖨 Print / Save as PDF</button>
  <span style="font-size:12px;color:#6b7280;align-self:center">Save as PDF using your browser's print dialog — choose "Save as PDF" as the destination.</span>
</div>
<h1>Financial Summary — ${esc(personName)}</h1>
<div class="meta">${esc(fy)} &nbsp;·&nbsp; Generated ${now} &nbsp;·&nbsp; <b>Confidential — not tax advice</b></div>

<h2>At a glance</h2>
<div class="stat-grid">
  <div class="stat good"><div class="l">Gross income</div><div class="v">${fmt(s.assessable)}</div></div>
  <div class="stat"><div class="l">Taxable income</div><div class="v">${fmt(s.taxable)}</div></div>
  <div class="stat bad"><div class="l">Est. tax payable</div><div class="v">${fmt(s.tax.total)}</div></div>
  <div class="stat ${s.balance>=0?'good':'bad'}"><div class="l">${s.balance>=0?'Est. refund':'Est. owing'}</div><div class="v">${fmt(Math.abs(s.balance))}</div></div>
</div>

<h2>Income</h2>
<table><thead><tr><th>Source</th><th>Type</th><th class="num">Yearly</th></tr></thead><tbody>
${incRows}
${divTotal?`<tr><td>Share dividends</td><td>Investment income</td><td class="num">${fmt(divTotal)}</td></tr>`:''}
${fundTotal?`<tr><td>ETF / Fund distributions</td><td>Investment income</td><td class="num">${fmt(fundTotal)}</td></tr>`:''}
<tr class="total"><td colspan="2">Total assessable income</td><td class="num">${fmt(s.assessable)}</td></tr>
</tbody></table>

${ptdRows?`<h2>Pre-tax deductions (salary sacrifice)</h2>
<table><thead><tr><th>Item</th><th>Type</th><th class="num">Yearly</th></tr></thead><tbody>
${ptdRows}
<tr class="total"><td colspan="2">Total pre-tax deductions</td><td class="num">${fmt(s.preTaxDeds||0)}</td></tr>
</tbody></table>`:''}

${dedRows?`<h2>Tax deductions</h2>
<table><thead><tr><th>Category</th><th class="num">Amount</th></tr></thead><tbody>
${dedRows}
<tr class="total"><td>Total deductions</td><td class="num">${fmt(s.deductions)}</td></tr>
</tbody></table>`:''}

<h2>Tax calculation (${esc(fy)} rates)</h2>
<table><tbody>
<tr><td>Taxable income</td><td class="num">${fmt(s.taxable)}</td></tr>
<tr><td>Income tax</td><td class="num">${fmt(s.tax.incomeTax||0)}</td></tr>
${s.tax.lowIncomeTaxOffset?`<tr><td>Low income tax offset (LITO)</td><td class="num">−${fmt(s.tax.lowIncomeTaxOffset)}</td></tr>`:''}
${s.tax.medicarelevy?`<tr><td>Medicare levy</td><td class="num">${fmt(s.tax.medicarelevy)}</td></tr>`:''}
<tr class="total"><td>Total tax payable</td><td class="num">${fmt(s.tax.total)}</td></tr>
<tr><td>Tax withheld (PAYG)</td><td class="num">${fmt(s.taxWithheld)}</td></tr>
<tr class="total"><td>${s.balance>=0?'Estimated refund':'Estimated amount owing'}</td><td class="num">${fmt(Math.abs(s.balance))}</td></tr>
</tbody></table>

<div class="disclaimer">⚠ These figures are estimates only, generated from a rate snapshot in Ledger. They are not a substitute for professional tax advice. Always verify against the ATO's figures before lodging your return. Effective rate shown: ${(s.effRate*100).toFixed(1)}%.</div>
</body></html>`;
  ...
  if(w){w.document.write(html);w.document.close();}
    ...
    a.href=url;a.download=`Ledger-${personName.replace(/\s+/g,'_')}-${fy}.html`;a.click();
    setTimeout(()=>URL.revokeObjectURL(url),5000);
    toast('Popup blocked — summary downloaded as HTML file');
  }
}
...
PAGES.summary=m=>{
  ...
  head(m,'FY summary',`Everything for ${esc(y.label)} in one statement, computed with that year's frozen rate snapshot.`,
    `<button class="btn" onclick="shareReport()">Share / Export</button>
     <button class="btn" onclick="mytaxExpandAll();setTimeout(()=>window.print(),250)">Print / PDF</button>`);
  ...
  <div class="grid2">
    ...
    <div>
      <div class="card"><div class="chead"><h2>Tax position</h2></div><div class="cbody">
        ...
        ${o.hasCover&&y.rates.phi?`<div class="fldrow" style="margin-bottom:12px;font-size:.83rem">
          <div><label class="fld">Annual PHI premium paid ($) <span class="muted">(optional — for rebate estimate)</span></label>
            <input class="input money" style="height:32px" value="${o.phiPremium||''}" placeholder="e.g. 2400" onchange="PD().summaryOpts.phiPremium=num(this.value);save();render()"></div>
          <div><label class="fld">Age group</label><select class="input" style="height:32px" onchange="PD().summaryOpts.phiAge=this.value;save();render()">
            <option value="under65" ${(o.phiAge||'under65')==='under65'?'selected':''}>Under 65</option>
            <option value="age65to69" ${o.phiAge==='age65to69'?'selected':''}>65–69</option>
            <option value="age70plus" ${o.phiAge==='age70plus'?'selected':''}>70+</option>
          </select></div>
        </div>`:''}
      ...
      <div class="card"><div class="chead"><h2>Refund estimate</h2></div><div class="cbody">
        ...
        ${s.phiRebate?`<div class="kv"><span class="k">PHI government rebate (est.)</span><span class="v" style="color:var(--euc)">${fmt$(s.phiRebate)}</span></div>`:''}
        ...
        <div class="note">An estimate only. PHI rebate above is calculated from the premium you enter — actual rebate depends on your income for MLS purposes and insurer tier. Verify all figures with the ATO before lodging.</div>
      </div></div>
    </div>
  </div>`);
};
...
function nwMonthSeries(pids,kindFilter,cutoffDate,startDate){
  ...
  let first=all.reduce((m,e)=>e.date<m?e.date:m,all[0].date).slice(0,7);
  // Clamp the chart's starting month to the FY's start when scoped to a
  // specific year, rather than always running from the earliest entry
  // ever recorded. Entries before this point are still considered below
  // (byItem isn't filtered by startDate) so the first month in range
  // correctly carries forward whatever balance was last recorded, rather
  // than starting from zero.
  if(startDate&&startDate.slice(0,7)>first)first=startDate.slice(0,7);
  const months=[];
...
PAGES.super=m=>{
  ...
  // Scope chart and balances to the selected FY (or all history / today for "all time")
  const {start:fyStart,end:fyEnd}=fyRange(y);
  const today=todayISO();
  const cutoff=isAllFY()?today:(fyEnd<today?fyEnd:today);
  const isFrozen=cutoff<today;

  const series=nwMonthSeries([pid],'super',cutoff,isAllFY()?null:fyStart);
...
PAGES.networth=m=>{
  ...
  const fullSeries=nwMonthSeries(pids,null,cutoff);
  const series=fullSeries.filter(s=>s.eom>=fyRange(y).start);
  const latest=fullSeries.length?fullSeries[fullSeries.length-1].value:0;
  const prevM=fullSeries.length>1?fullSeries[fullSeries.length-2].value:null;
  ...
  const seriesOneYr=fullSeries.filter(s=>s.eom<=oneYrAgo);
  ...
  // All-time change
  const allTimeFirst=fullSeries.length?fullSeries[0].value:null;
  const dltAll=allTimeFirst!=null&&fullSeries.length>1?latest-allTimeFirst:null;
...
PAGES.assetDetail=m=>{
  ...
  head(m,`${emoji} ${esc(a.name)}`,`Details — ${scoped?esc(fyDisplay(FY())):'all time'}${a.code?` · ${esc(a.code)}`:''}`,
    `${(a.kind==='shares'||a.kind==='managed_fund')&&!isAll()
      ?`<button class="btn small" onclick="assetTxAdd('${a.id}')">+ Buy / Sell</button>
         <button class="btn ghost small" onclick="assetUpdatePosition('${a.id}')">↻ Update position</button>`
      :''}
     ${a.code&&!isAll()?`<button class="btn ghost small" onclick="assetFetchPrice('${a.id}')">↻ Refresh price</button>`:''}
...
let ASSET_PROP_EXP_COLLAPSED=new Set(); // asset ids with property expenses collapsed, transient
let ASSET_TX_EXPANDED=new Set(); // asset ids with transactions expanded (collapsed by default), transient
let ASSET_SELECTED=new Set(); // ids of assets ticked for bulk delete, transient
// Groups start collapsed — user expands what they want
let ASSET_GROUP_COLLAPSED=new Set(['shares','managed_fund','property','vehicle','super','savings','offset','other','ff_points']);
const ASSET_GROUPS=[
  ['shares','📈 Shares'],
...
const CGT_REFORM_DATE='2027-07-01'; // Proposed commencement — not yet law as of Jun 2026
function cgtMarginalRate(pid){
  const y=FY(),s=fySummaryNumbers(y,pid);
  let r=0;for(const br of y.rates.brackets){if(s.taxable>=(br.upTo||Infinity))r=br.rate;else{r=br.rate;break;}}
  return r;
}
function cgtPlanner(assetId){
  const a=DB.assets.find(x=>x.id===assetId);if(!a)return;
  const pid=a.pid||DB.people[0].id;
  const heldQty=assetHeldQty(a);
  const lastPrice=a.lastPrice?.price||0;
  const buys=(a.transactions||[]).filter(t=>t.type==='buy'||t.type==='drp'||t.type==='base').sort((x,y)=>x.date<y.date?-1:1);
  const earliestBuy=buys.length?buys[0].date:'';
  const twelveMonthsAgo=new Date();twelveMonthsAgo.setFullYear(twelveMonthsAgo.getFullYear()-1);
  const discountEligible=earliestBuy&&new Date(earliestBuy+'T00:00:00Z')<twelveMonthsAgo;
  const mRate=cgtMarginalRate(pid);
  // Use stored 2027 valuation if confirmed, else fall back to current price
  const stored2027=a.cgt2027Value||lastPrice||0;
  modal('📊 What if I sell? — '+esc(a.name),
    '<div class="hint" style="margin-bottom:12px">Shows two scenarios — current tax rules (for a sale <b>before 1 Jul 2027</b>) and the Government\'s proposed new rules (for a sale <b>after 1 Jul 2027</b>). The new rules have not yet passed Parliament.</div>'
    +'<div class="fldrow">'
    +'<div><label class="fld">Units to sell</label><input id="cgt_qty" class="input money" value="'+heldQty+'" oninput="cgtPlannerCalc(\''+assetId+'\')"></div>'
    +'<div><label class="fld">Sale price per unit ($)</label><input id="cgt_price" class="input money" value="'+(lastPrice||'')+'" oninput="cgtPlannerCalc(\''+assetId+'\')"></div>'
    +'<div><label class="fld">Value/unit at 1 Jul 2027 ($) <span class="muted">(for new rules)</span></label><input id="cgt_2027price" class="input money" value="'+(stored2027||'')+'" placeholder="e.g. '+(lastPrice?fmt$(lastPrice):'market price on that date')+'" oninput="cgtPlannerCalc(\''+assetId+'\')">'
    +(a.cgt2027Confirmed?'<div style="font-size:.72rem;color:var(--euc);margin-top:3px">✓ Confirmed valuation recorded</div>':'<div style="font-size:.72rem;color:var(--muted);margin-top:3px">Using current price as proxy — <a href="#" onclick="cgt2027Record(\''+assetId+'\',true);return false">record an actual valuation</a></div>')
    +'</div></div>'
    +'<div id="cgt_result" style="margin-top:16px"></div>'
    +'<div class="note mt">Earliest buy: <b>'+(earliestBuy?fmtDate(earliestBuy):'unknown')+'</b> · '+(discountEligible?'<span style="color:var(--euc)">held >12 months</span>':'not yet held 12 months')+' · marginal rate: <b>'+mRate+'%</b>. Estimates only — not tax advice. New rules legislation is <b>Treasury Laws Amendment (Tax Reform No.1) Bill 2026</b>, introduced 28 May 2026, not yet passed.</div>',
    '<button class="btn" data-close>Close</button>');
  setTimeout(()=>cgtPlannerCalc(assetId),50);
}
function cgtPlannerCalc(assetId){
  const a=DB.assets.find(x=>x.id===assetId);if(!a)return;
  const pid=a.pid||DB.people[0].id;
  const heldQty=assetHeldQty(a);const costBasis=assetCostBasis(a);
  const avgCostPerUnit=heldQty?costBasis/heldQty:0;
  const qty=Math.min(num($('#cgt_qty')?.value)||0,heldQty);
  const price=num($('#cgt_price')?.value);
  const price2027=num($('#cgt_2027price')?.value)||price; // fall back to sale price if not set
  const el=$('#cgt_result');if(!el)return;
  if(!qty||!price){el.innerHTML='<div class="muted" style="font-size:.88rem">Enter units and price to see the estimate.</div>';return;}

  const proceeds=qty*price;
  const costForUnits=qty*avgCostPerUnit;
  const rawGain=proceeds-costForUnits;
  const isLoss=rawGain<0;

  const buys=(a.transactions||[]).filter(t=>t.type==='buy'||t.type==='drp'||t.type==='base').sort((x,y)=>x.date<y.date?-1:1);
  const earliestBuy=buys.length?buys[0].date:'';
  const twelveMonthsAgo=new Date();twelveMonthsAgo.setFullYear(twelveMonthsAgo.getFullYear()-1);
  const discountEligible=rawGain>0&&earliestBuy&&new Date(earliestBuy+'T00:00:00Z')<twelveMonthsAgo;
  const mRate=cgtMarginalRate(pid);

  const row=(label,val,style)=>'<div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px dashed var(--line);font-size:.84rem;'+(style||'')+'"><span style="color:var(--muted)">'+label+'</span><span style="font-variant-numeric:tabular-nums;font-weight:var(--fw-semibold)">'+val+'</span></div>';
  const taxRow=(val)=>row('Estimated additional tax',fmt$0(val),'font-size:.9rem;font-weight:bold');

  // ── Scenario A: Current rules (sale before 1 Jul 2027) ──
  const discountedGainA=discountEligible?rawGain*0.5:rawGain;
  const taxableA=Math.max(0,discountedGainA);
  const taxA=taxableA*(mRate/100);

  let htmlA='<div class="card" style="border-color:var(--euc)">'
    +'<div class="chead" style="background:color-mix(in srgb,var(--euc-soft) 30%,var(--surface))"><h2 style="font-size:.9rem">✅ Current rules — sell before 1 Jul 2027</h2></div>'
    +'<div class="cbody">'
    +row('Proceeds',fmt$0(proceeds),'')
    +row('Cost basis',fmt$0(costForUnits),'')
    +row(isLoss?'Capital loss':'Capital gain',fmt$0(Math.abs(rawGain)),isLoss?'color:var(--euc)':'')
    +(isLoss
      ?'<div style="padding:8px 0;font-size:.84rem;color:var(--euc)">Capital loss — no tax. Offsets other capital gains.</div>'
      :(discountEligible?row('After 50% discount',fmt$0(discountedGainA),'color:var(--euc)'):'')+taxRow(taxA)
       +'<div style="font-size:.73rem;color:var(--muted);margin-top:4px">'+mRate+'% marginal rate'+(discountEligible?' on '+fmt$0(taxableA)+' (after 50% discount)':'')+'</div>')
    +'</div></div>';

  // ── Scenario B: Proposed new rules (sale after 1 Jul 2027) ──
  // Split gain: pre-2027 portion uses 50% discount, post-2027 uses indexation + 30% min tax
  const value2027ForUnits=qty*price2027;        // what the units were worth at 1 Jul 2027
  const preGain=value2027ForUnits-costForUnits; // gain accrued before 1 Jul 2027
  const postGain=proceeds-value2027ForUnits;    // gain accrued after 1 Jul 2027

  // Pre-2027 portion: 50% discount if held >12 months at that date
  const preGainTaxable=discountEligible&&preGain>0?preGain*0.5:Math.max(0,preGain);
  const preTax=preGainTaxable*(mRate/100);

  // Post-2027 portion: indexation (approximate with user-adjustable CPI — we use 2.5% default)
  // Indexed cost base = value2027 × (1 + CPI)^holdingYears (approximate post-2027 period)
  // For simplicity in the planner we show the raw post-2027 gain with the 30% min tax floor
  const cpiRate=0.025; // 2.5% assumed CPI — close to RBA target midpoint
  // Rough years held post-2027: from 2027-07-01 to "now" (sale date unknown, we approximate 1yr)
  const postIndexedCost=value2027ForUnits*(1+cpiRate); // 1 year indexation as approximation
  const postGainIndexed=Math.max(0,proceeds-postIndexedCost);
  const postTaxAtMarginal=postGainIndexed*(mRate/100);
  const postTaxMin30=postGain>0?postGain*0.30:0; // 30% minimum on full (non-indexed) post gain
  const postTax=Math.max(postTaxAtMarginal,postTaxMin30);
  const totalTaxB=preTax+(postGain>0?postTax:0);
  const totalTaxableB=preGainTaxable+(postGain>0?postGainIndexed:0);

  let htmlB='<div class="card" style="border-color:var(--gold-badge);margin-top:10px">'
    +'<div class="chead" style="background:color-mix(in srgb,var(--gold-soft) 30%,var(--surface))"><h2 style="font-size:.9rem">⚠️ Proposed new rules — sell after 1 Jul 2027 <span style="font-weight:400;font-size:.78rem;color:var(--muted)">(not yet law)</span></h2></div>'
    +'<div class="cbody">'
    +'<div style="font-size:.78rem;color:var(--muted);margin-bottom:8px">Gain split at 1 Jul 2027: 50% discount on pre-2027 gains, CPI indexation + 30% min tax on post-2027 gains.</div>';

  if(rawGain<=0){
    htmlB+='<div style="padding:8px 0;font-size:.84rem;color:var(--euc)">Capital loss — no tax under either regime.</div>';
  }else{
    htmlB+=row('Pre-2027 gain (50% discount applies)',fmt$0(preGain>0?preGain:0),'')
      +row('Post-2027 gain (indexation + 30% min)',fmt$0(postGain>0?postGain:0),'')
      +(preGain>0&&discountEligible?row('Pre-2027 taxable (after 50% discount)',fmt$0(preGainTaxable),'color:var(--euc)'):'')
      +(postGain>0?row('Post-2027 taxable (indexed cost base, ~2.5% CPI)',fmt$0(postGainIndexed),''):'')
      +(postGain>0?row('30% minimum check',fmt$(postGain*0.30)+' min vs '+fmt$(postTaxAtMarginal)+' at '+mRate+'%','font-size:.78rem'):'')
      +taxRow(totalTaxB>0?totalTaxB:0)
      +'<div style="font-size:.73rem;color:var(--muted);margin-top:4px">Based on ~1yr post-2027 holding at 2.5% CPI. Actual tax depends on sale date, CPI and final legislation.</div>';
  }

  const saving=taxA-totalTaxB;
  htmlB+='</div></div>';

  if(!isLoss&&taxA>0&&totalTaxB>0){
    const diff=Math.abs(saving);
    htmlB+='<div class="note mt" style="border-color:'+(saving>0?'var(--euc)':'var(--red)')+'">💡 Under the proposed rules you would pay <b>'+fmt$0(diff)+'</b> '+(saving>0?'<span style="color:var(--euc)">less</span>':'<span style="color:var(--red)">more</span>')+' tax on this sale compared to current rules. Selling before 1 July 2027 may be worth considering if the current rules result in a lower tax bill.</div>';
  }

  el.innerHTML=htmlA+htmlB;
}
/* Record the confirmed CGT 2027 valuation on an asset */
function cgt2027Record(assetId,fromPlanner){
  const a=DB.assets.find(x=>x.id===assetId);if(!a)return;
  const isShare=a.kind==='shares'||a.kind==='managed_fund';
  const currentVal=a.lastPrice?.price||0;
  const stored=a.cgt2027Value||currentVal||'';
  const label=isShare?'Price per unit at 1 July 2027 ($)':'Market value at 1 July 2027 ($)';
  const hint=isShare
    ?'Enter the share/unit price on 1 July 2027. For shares/ETFs this will be available on your broker platform or from the ASX. This becomes the cost base for the new CGT rules on any post-2027 gains.'
    :'For investment properties and vehicles, you\'ll need a formal independent valuation dated as close to 1 July 2027 as possible. Keep the valuation report — the ATO may request it.';
  modal('Record 1 July 2027 valuation — '+esc(a.name),
    '<div class="hint" style="margin-bottom:12px">'+hint+'</div>'
    +'<div class="fldrow"><div><label class="fld">'+label+'</label>'
    +'<input id="cgt27_val" class="input money" value="'+(stored||'')+'" placeholder="e.g. '+fmt$(currentVal||0)+'"></div></div>'
    +'<div class="fldrow mt"><div><label class="fld">How was this value determined?</label>'
    +'<select id="cgt27_method" class="input">'
    +'<option value="broker"'+(a.cgt2027Method==='broker'?' selected':'')+'>Broker / market price on the day</option>'
    +'<option value="ato_tool"'+(a.cgt2027Method==='ato_tool'?' selected':'')+'>ATO valuation tool</option>'
    +'<option value="valuation"'+(a.cgt2027Method==='valuation'?' selected':'')+'>Independent professional valuation</option>'
    +'<option value="estimate"'+(a.cgt2027Method==='estimate'?' selected':'')+'>My own estimate (update when final price known)</option>'
    +'</select></div></div>',
    '<button class="btn" data-close>Cancel</button>'
    +'<button class="btn primary" onclick="cgt2027RecordSave(\''+assetId+'\','+(fromPlanner?'true':'false')+')">Save valuation</button>');
}
function cgt2027RecordSave(assetId,fromPlanner){
  const a=DB.assets.find(x=>x.id===assetId);if(!a)return;
  const val=num($('#cgt27_val')?.value);
  const method=$('#cgt27_method')?.value||'estimate';
  if(!val)return toast('Enter a value first');
  a.cgt2027Value=val;
  a.cgt2027Method=method;
  a.cgt2027Confirmed=method!=='estimate';
  save();closeModal();
  if(fromPlanner)cgtPlanner(assetId);
  else render();
  toast('Valuation recorded'+(a.cgt2027Confirmed?'':' — mark as confirmed when you have the final figure'));
}
...
/* ---- Reusable "more actions" overflow menu (asset rows etc.) — mirrors
   the _cselOpen pattern used by the custom select dropdown: single open-id,
   toggle with stopPropagation so the trigger click doesn't immediately
   re-close it via the document-level listener below. ---- */
let ACTMENU_OPEN=null;
function actMenuToggle(id,ev){
  if(ev)ev.stopPropagation();
  ACTMENU_OPEN=ACTMENU_OPEN===id?null:id;
  render();
}
document.addEventListener('click',()=>{if(ACTMENU_OPEN){ACTMENU_OPEN=null;render();}});
...
function assetShareCard(a){
  ...
  return `<div class="card${a.closedDate?' archived':''}"><div class="chead">
    ...
    <span class="actions">
      ...
      <div class="actmenu">
        <button class="btn small ghost" onclick="actMenuToggle('share-${a.id}',event)" aria-haspopup="true" aria-expanded="${ACTMENU_OPEN==='share-'+a.id}" title="More actions">⋯</button>
        ${ACTMENU_OPEN==='share-'+a.id?`<div class="actmenu-panel" onclick="event.stopPropagation()">
          ${a.code&&!isAll()?`<button onclick="ACTMENU_OPEN=null;render();assetFetchPrice('${a.id}')">↻ Refresh live price</button>`:''}
          ${isAll()?'':`<button onclick="ACTMENU_OPEN=null;render();assetBaseAdd('${a.id}')">${(a.transactions||[]).some(t=>t.type==='base')?'Edit opening position':'📍 Set opening position'}</button>`}
          ${isAll()?'':`<button onclick="ACTMENU_OPEN=null;render();assetUpdatePosition('${a.id}')" title="Set new total units + average cost in one go, instead of logging each buy">↻ Update position</button>`}
          ${heldQty>0&&!isAll()?`<button onclick="ACTMENU_OPEN=null;render();cgtPlanner('${a.id}')">📊 What if I sell?</button>`:''}
          ${isAll()?'':`<hr>${a.closedDate?`<button onclick="ACTMENU_OPEN=null;render();assetReopenPosition('${a.id}')">Reopen</button>`:`<button onclick="ACTMENU_OPEN=null;render();assetClosePosition('${a.id}')">Close position</button>`}`}
          <hr><button class="danger" onclick="ACTMENU_OPEN=null;render();confirmDel('Delete ${esc(a.name)}? This also deletes all its transactions, holdings, linked net worth items, and every dividend/distribution recorded against it across all financial years.',()=>assetDelete('${a.id}'))">✕ Delete</button>
        </div>`:''}
      </div>
    </span></div>
    <div class="cbody">
      ...
      ${hasFYBuys?`<div style="margin-bottom:14px">${barChartSVG(buysByMonth,{aria:'Purchases this FY by month'})}<div class="hint" style="margin-top:4px">Buy value per month in ${esc(fyDisplay(FY()))}</div></div>`:''}
      <div class="section-toggle" onclick="if(ASSET_TX_EXPANDED.has('${a.id}'))ASSET_TX_EXPANDED.delete('${a.id}');else ASSET_TX_EXPANDED.add('${a.id}');render()" style="cursor:pointer;display:flex;align-items:center;justify-content:space-between;padding:8px 0;border-top:1px solid var(--line);margin-top:8px;user-select:none">
        <span style="font-size:.9rem;font-weight:700">Transactions <span class="muted" style="font-weight:400">(${txs.length})</span></span>
        <span style="display:flex;align-items:center;gap:10px;font-size:.82rem;color:var(--muted)">
          ${!ASSET_TX_EXPANDED.has(a.id)&&txs.length?`<span>${heldQty} units · avg ${heldQty?fmt$(costBasis/heldQty):'—'}/unit</span>`:''}
          <span style="font-size:.75rem">${ASSET_TX_EXPANDED.has(a.id)?'▲ collapse':'▾ expand'}</span>
        </span>
      </div>
      ${ASSET_TX_EXPANDED.has(a.id)
        ?`<div style="margin-bottom:8px">${tRows
            ?`<table class="tbl"><thead><tr><th>Type</th><th>Date</th><th class="num">Qty</th><th class="num">Price</th><th class="num">Fees</th><th class="num">Total</th><th></th></tr></thead><tbody>${tRows}</tbody></table>`
            :`<div class="muted" style="padding:8px 0">No transactions yet — add a buy to start tracking this holding.</div>`
          }</div>`
        :''}
      ${a.kind==='managed_fund'||a.kind==='shares'?`
      <div class="section-toggle" onclick="if(ASSET_TX_EXPANDED.has('div-${a.id}'))ASSET_TX_EXPANDED.delete('div-${a.id}');else ASSET_TX_EXPANDED.add('div-${a.id}');render()" style="cursor:pointer;display:flex;align-items:center;justify-content:space-between;padding:8px 0;border-top:1px solid var(--line);margin-top:4px;user-select:none">
        <span style="font-size:.9rem;font-weight:700">Linked ${a.kind==='managed_fund'?'distributions':'dividends'} <span class="muted" style="font-weight:400">(${(a.kind==='managed_fund'?linkedFP:linkedDivs).length})</span></span>
        <span style="font-size:.75rem;color:var(--muted)">${ASSET_TX_EXPANDED.has('div-'+a.id)?'▲ collapse':'▾ expand'}</span>
      </div>
      ${ASSET_TX_EXPANDED.has('div-'+a.id)?`<div style="margin-bottom:8px">${(a.kind==='managed_fund'?linkedFP:linkedDivs).length
        ...
        :`<div class="muted" style="padding:8px 0">None yet — go to ${a.kind==='managed_fund'?`<a href="#" onclick="go('funds');return false">ETF / Managed Funds</a>`:`<a href="#" onclick="go('dividends');return false">Share Dividends</a>`} to link ${a.kind==='managed_fund'?'distributions':'dividends'} to this asset.</div>`
        }</div>`:''}
      `:''}
    </div></div>`;
}
...
function assetBaseAdd(assetId){
  ...
  modal(existing?'Edit opening position':'Set opening position',`
    ...
    `<button class="btn" data-close>Cancel</button>${existing?`<button class="btn ghost" onclick="confirmDel('Remove the opening position? Any buys/sells already logged will remain.',()=>{const a=DB.assets.find(x=>x.id==='${assetId}');a.transactions=a.transactions.filter(x=>x.type!=='base');save();closeModal();render()})">Remove</button>`:''}<button class="btn primary" onclick="assetBaseSave('${assetId}')">Save</button>`);
}
...
function assetCard(a){
  ...
  return `<div class="card${a.closedDate?' archived':''}"><div class="chead"><h2>${isAll()?'':`<input type="checkbox" ${ASSET_SELECTED.has(a.id)?'checked':''} onchange="assetToggleSelect('${a.id}',this.checked)" title="Select for bulk delete" style="margin-right:6px;vertical-align:middle">`}${isAll()?pdot(person(a.pid||DB.people[0].id))+' ':''}${emoji} ${esc(a.name)}${a.kind==='property'&&a.investment===false?' <span class="badge">Home</span>':''}${a.closedDate?` <span class="badge gold">closed ${fmtDate(a.closedDate)}</span>`:''}</h2><span class="actions">
      ...
      <button class="btn small" onclick="assetAdd('${a.id}')">Edit</button>
      <div class="actmenu">
        <button class="btn small ghost" onclick="actMenuToggle('asset-${a.id}',event)" aria-haspopup="true" aria-expanded="${ACTMENU_OPEN==='asset-'+a.id}" title="More actions">⋯</button>
        ${ACTMENU_OPEN==='asset-'+a.id?`<div class="actmenu-panel" onclick="event.stopPropagation()">
          ${isAll()?'':`${a.closedDate?`<button onclick="ACTMENU_OPEN=null;render();assetReopenPosition('${a.id}')">Reopen</button>`:`<button onclick="ACTMENU_OPEN=null;render();assetClosePosition('${a.id}')">Close position</button>`}<hr>`}
          <button class="danger" onclick="ACTMENU_OPEN=null;render();confirmDel('Delete ${esc(a.name)} and its linked net worth items (valuation, loan, and their value history)? This does not delete Investment Property expenses already recorded in past financial years.',()=>assetDelete('${a.id}'))">✕ Delete</button>
        </div>`:''}
      </div></span></div>
    <div class="cbody">
      <div class="grid3">
      ...
      ${a.kind==='property'&&a.investment===false?`<div style="display:flex;justify-content:space-between;align-items:center;cursor:pointer;padding:8px 0;border-top:1px solid var(--line);margin-top:8px" onclick="if(ASSET_PROP_EXP_COLLAPSED.has('${a.id}'+'h'))ASSET_PROP_EXP_COLLAPSED.delete('${a.id}'+'h');else ASSET_PROP_EXP_COLLAPSED.add('${a.id}'+'h');render()">
        <h3 style="font-size:.92rem;margin:0"><span style="display:inline-block;width:1em">${ASSET_PROP_EXP_COLLAPSED.has(a.id+'h')?'▸':'▾'}</span>Expenses</h3>
        <span class="muted" style="font-size:.8rem">${ASSET_PROP_EXP_COLLAPSED.has(a.id+'h')?'▾ expand':''}</span>
      </div>
      ${ASSET_PROP_EXP_COLLAPSED.has(a.id+'h')?'':`${assetPropertyExpenses(a)}
      <div class="note mt">This is set as a home (not rented out), so none of these expenses are tax-deductible — tracked here for your own budgeting only.</div>`}`:`
      <div style="display:flex;justify-content:space-between;align-items:center;cursor:pointer;padding:8px 0;border-top:1px solid var(--line);margin-top:8px;user-select:none" onclick="if(ASSET_PROP_EXP_COLLAPSED.has('${a.id}'))ASSET_PROP_EXP_COLLAPSED.delete('${a.id}');else ASSET_PROP_EXP_COLLAPSED.add('${a.id}');render()">
        <h3 style="font-size:.92rem;margin:0"><span style="display:inline-block;width:1em">${ASSET_PROP_EXP_COLLAPSED.has(a.id)?'▸':'▾'}</span>Expenses${isAllFY()?' by financial year':' — '+esc(fyDisplay(FY()))}</h3>
        <span class="muted" style="font-size:.8rem">${ASSET_PROP_EXP_COLLAPSED.has(a.id)?'▾ expand':''}</span>
      </div>
      ${ASSET_PROP_EXP_COLLAPSED.has(a.id)?'':a.kind==='property'||a.kind==='vehicle'?assetPropertyExpenses(a):`<div class="note">Expenses for this asset are tracked per FY in the Expenses page.</div>`}
      ${ASSET_PROP_EXP_COLLAPSED.has(a.id)?'':a.kind==='property'?(isAllFY()
        ...
        :(()=>{
          let exp=0;DB.people.forEach(pp=>{exp+=bucket(FY(),pp.id).property.expenses.reduce((s,e)=>s+yearlyFromRec(num(e.amount),e.recurrence||'once'),0);});
...
function assetDepreciationSection(a){
  ...
  const depReportBtn=isAll()?'':(a.depReportId
    ?`<button class="btn ghost small" onclick="event.stopPropagation();rcptView('${a.depReportId}','${esc(a.depReportName||'Depreciation report')}')" title="View attached depreciation schedule">📎 ${esc(a.depReportName||'Report')}</button>
      <button class="btn ghost small" onclick="event.stopPropagation();assetDepReportRemove('${a.id}')" title="Remove attached file">Remove</button>`
    :`<button class="btn ghost small" onclick="event.stopPropagation();assetDepReportAttach('${a.id}')">📎 Attach report</button>`);
  return `<div class="mt">
    <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;margin-bottom:8px;cursor:pointer" onclick="assetDepToggle('${a.id}')">
      ...
      <span style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
        ...
        ${isAll()||!expanded?'':`${depReportBtn}
        <button class="btn ghost small" onclick="event.stopPropagation();assetDepBulkAdd('${a.id}')">+ Bulk add years</button>
...
async function assetDepReportAttach(assetId){
  const a=DB.assets.find(x=>x.id===assetId);if(!a)return;
  // Use a file input — created and clicked programmatically
  const inp=document.createElement('input');
  inp.type='file';inp.accept='image/*,.pdf';
  inp.onchange=async()=>{
    const file=inp.files[0];if(!file)return;
    const oldId=a.depReportId;
    a.depReportId=uid();a.depReportName=file.name;
    save();render();
    toast('Attaching depreciation report…');
    try{
      await rcptPut({id:a.depReportId,name:file.name,type:file.type,blob:file},
        {fy:fyDisplay(FY()),category:'Depreciation report',date:todayISO(),itemName:esc(a.name)+' — depreciation schedule',pid:DB.currentPid});
      if(oldId&&!receiptStillReferenced(oldId))rcptDel(oldId).catch(()=>{});
      toast('Depreciation report attached');render();
    }catch(e){toast('Could not attach file — try again');}
  };
  inp.click();
}
function assetDepReportRemove(assetId){
  const a=DB.assets.find(x=>x.id===assetId);if(!a)return;
  confirmDel('Remove the attached depreciation report?',()=>{
    const oldId=a.depReportId;
    a.depReportId='';a.depReportName='';
    save();render();
    if(oldId&&!receiptStillReferenced(oldId))rcptDel(oldId).catch(()=>{});
  });
}
...
function assetPropertyExpenses(a){
  ...
  cats.filter(c=>byCat[c]).forEach(c=>{
    const ct=byCat[c].reduce((s,e)=>s+yearlyFromRec(num(e.amount),e.recurrence||'once'),0);
    ...
    body+=byCat[c].map(e=>{
      ...
      const yearlyAmt=isRecurring?yearlyFromRec(full,e.recurrence):full;
      const recLabel=isRecurring?` <span class="muted" style="font-size:.72rem">${RECURRENCE_OPTS.find(([v])=>v===e.recurrence)?.[1]||e.recurrence}</span>`:'';
      const amtDisplay=isRecurring?`${fmt$(full)}<span class="muted">/period</span>`:fmt$(full);
      ...
      return `<tr><td></td><td>${fmtDate(e.date)}${recLabel}</td><td>${esc(e.item||'')}</td>
...
PAGES.settings=m=>{
  ...
  <div class="grid2">
    <div>
      ...
      <div class="card"><div class="chead"><h2>Low income tax offset</h2></div><div class="cbody">
        <div class="fldrow">
          ...
          <div><label class="fld">Full offset until $</label><input class="input money" ${dis} value="${R.lito.t1}" onchange="FY().rates.lito.t1=num(this.value);save()"></div>
        </div>
        <div class="fldrow mt">
          <div><label class="fld">Taper 1 % (above that)</label><input class="input money" ${dis} value="${R.lito.taper1}" onchange="FY().rates.lito.taper1=num(this.value);save()"></div>
        ...
        <div class="fldrow mt">
          <div><label class="fld">Taper 2 % (beyond that, to zero)</label><input class="input money" ${dis} value="${R.lito.taper2}" onchange="FY().rates.lito.taper2=num(this.value);save()"></div>
        </div>
        <div class="hint mt">Full ${fmt$0(R.lito.max)} offset up to ${fmt$0(R.lito.t1)} taxable income, then tapers at ${R.lito.taper1}% to ${fmt$0(R.lito.t2)}, then ${R.lito.taper2}% until fully phased out.</div>
      </div></div>
    ...
    <div>
      <div class="card"><div class="chead"><h2>WFH & travel rates</h2></div><div class="cbody">
        <div class="fldrow">
...
└── FY24–25/
    └── …</pre>
      ...
      <div class="card"><div class="chead"><h2>Sharing & household access</h2></div><div class="cbody">
        ${DB.gdrive?.sharedFileId?`
          <div class="note" style="border-color:var(--euc);margin-bottom:12px">✓ Connected to a shared file (ID: <code style="font-size:.78rem">${esc(DB.gdrive.sharedFileId.slice(0,20))}…</code>). All changes sync to the owner's Google Drive file.</div>
          <button class="btn ghost small" onclick="sharingDisconnect()">Disconnect from shared file</button>
          <div class="hint mt">To stop sharing, disconnect here. Your own data (if any) remains in your Drive separately.</div>
        `:`
          <p style="font-size:.85rem;color:var(--muted);margin-bottom:14px;line-height:1.55">Share your Ledger with a household member so they can view and edit the same data on their own device.</p>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
            <div style="border:1px solid var(--line);border-radius:8px;padding:14px">
              <div style="font-weight:var(--fw-semibold);margin-bottom:4px">Step 1 — Share your file <span class="badge euc" style="font-size:.68rem">your side</span></div>
              <div style="font-size:.82rem;color:var(--muted);margin-bottom:10px">Enter their Google account email and we'll give them edit access to your Ledger data file in Drive.</div>
              ${GD.isConnected()?`
                <input id="share_email" class="input" type="email" placeholder="partner@gmail.com" style="margin-bottom:8px">
                <button class="btn primary small" onclick="sharingGrantAccess()">Share with this email</button>
                <div id="share_status" style="font-size:.78rem;margin-top:6px"></div>
              `:`<div class="muted" style="font-size:.83rem">Connect Google Drive first (above) to share your file.</div>`}
            </div>
            <div style="border:1px solid var(--line);border-radius:8px;padding:14px">
              <div style="font-weight:var(--fw-semibold);margin-bottom:4px">Step 2 — Connect to shared file <span class="badge blue" style="font-size:.68rem">partner's side</span></div>
              <div style="font-size:.82rem;color:var(--muted);margin-bottom:10px">After the owner shares the file, enter the file ID they send you. All your edits will go to their file.</div>
              <input id="share_fileid" class="input" placeholder="Drive file ID (from owner)" style="margin-bottom:8px">
              <button class="btn small" onclick="sharingConnectToFile()">Connect to this file</button>
              <div class="hint" style="margin-top:6px">The owner can find their file ID by clicking "Share your file" above and copying what appears.</div>
            </div>
          </div>
          <div class="note mt">Both people need to be signed in with their own Google account. The owner's file is shared via Google Drive — the partner's account gets edit permission, and their copy of Ledger reads/writes the same file.</div>
        `}
      </div></div>
      ...
      <div class="card"><div class="chead"><h2>Financial years</h2></div><div class="cbody">
        ${Object.values(DB.years).sort((a,b)=>fyOrderYear(b)-fyOrderYear(a)).map(fy=>`<div class="kv" style="${fy.archived?'opacity:.6':''}"><span class="k">${esc(fyDisplay(fy))}${fy.displayName?` <span class="muted" style="font-size:.78rem">(${esc(fy.label)})</span>`:''} ${fy.locked?'🔒':''}${fy.archived?' <span class="badge" style="font-size:.68rem">📦 archived</span>':''}</span>
          <span style="display:flex;gap:6px">
            <button class="btn ghost small" onclick="settingsEditFY('${fy.startYear}')">rename</button>
            <button class="btn ghost small" onclick="settingsArchiveFY('${fy.startYear}')">${fy.archived?'unarchive':'archive'}</button>
        ...
        <div class="note">"Clone → new FY" copies this year's rates as the starting point for the next year. Use <b>rename</b> to give any year a custom display name or adjust its start/end dates. <b>Archiving</b> hides a year from the financial year picker — its data is untouched and still counted everywhere else (Trends, Net worth history, "All time" totals); unarchive any time to bring it back into the picker.</div>
      </div></div>
...
/* Archiving hides a year from the FY picker dropdown without touching its
   data — everything (Trends, Net worth history, "All time" totals, "Clone
   → new FY", "Create from FY data") still sees it exactly as before. It's
   purely a decluttering toggle for the picker, fully reversible, and
   intentionally independent of locking (a year can be archived without
   being locked, or locked without being archived). */
function settingsArchiveFY(k){
  const y=DB.years[k];if(!y)return;
  y.archived=!y.archived;
  if(y.archived&&DB.currentFY===k){
    // Currently viewing the year being archived — switch to the most
    // recent remaining non-archived year rather than leaving the app
    // pointed at a year its own dropdown no longer lists.
    const remaining=Object.values(DB.years).filter(yy=>!yy.archived&&String(yy.startYear)!==k).sort((a,b)=>fyOrderYear(b)-fyOrderYear(a));
    DB.currentFY=remaining.length?String(remaining[0].startYear):'all';
  }
  save();initFYSelect();render();
  toast(y.archived?`${fyDisplay(y)} archived — hidden from the financial year picker`:`${fyDisplay(y)} unarchived`);
}
...
async function settingsDeletePersonGo(pid){
  ...
  Object.values(DB.years).forEach(y=>{delete (y.people||{})[pid];(y.budgets||[]).forEach(b=>{
    ...
    // A surviving budget's expense rows aren't person-owned, but the
    // deductible portion can be *claimed by* a specific person (dedPid) —
    // clear that if it pointed at the now-deleted person, so the
    // deduction's tax effect falls back to a remaining person instead of
    // silently disappearing (budTotals only applies it to a pid that's
    // still in this budget's people).
    (b.exps||[]).forEach(r=>{if(r.dedPid===pid)delete r.dedPid;});
    // Combined budgets link to each person's individual budget by pid —
    // drop the deleted person's entry so it doesn't keep pointing at a
    // budget that no longer exists.
    if(b.isCombined&&b.sources)delete b.sources[pid];
...
async function importAll(file){
  try{
    ...
    if(OFFLINE_MODE){showApp();toast('Backup restored — you\'re in offline mode');}
    else{initFYSelect();render();toast('Backup restored');}
...
async function sharingGrantAccess(){
  const email=$('#share_email')?.value.trim();
  const status=$('#share_status');
  if(!email||!email.includes('@')){toast('Enter a valid email address');return;}
  if(status)status.textContent='Looking up your data file…';
  try{
    const fileId=await GD.getDataFileId();
    if(!fileId){if(status)status.textContent='No data file found — save something first.';return;}
    if(status)status.textContent='Sharing…';
    await GD.shareFileWith(fileId,email);
    if(status)status.innerHTML=`✓ Shared! Tell ${esc(email)} to enter this file ID in their Ledger → Settings → Sharing:<br><code style="font-size:.85rem;user-select:all;background:var(--surface2);padding:4px 8px;border-radius:4px;display:inline-block;margin-top:6px">${esc(fileId)}</code>`;
    // Copy file ID to clipboard
    try{await navigator.clipboard.writeText(fileId);toast('File ID copied to clipboard — paste it to '+email);}catch(e){toast('File ID shown below — copy and send to '+email);}
  }catch(e){
    if(status)status.textContent='Error: '+e.message;
    toast('Could not share: '+e.message);
  }
}
async function sharingConnectToFile(){
  const fileId=$('#share_fileid')?.value.trim();
  if(!fileId){toast('Paste the file ID from the owner');return;}
  if(!GD.isConnected()){toast('Connect your Google Drive first (above)');return;}
  // Validate the file ID by trying to read it
  try{
    toast('Connecting…');
    const text=await GD.readJSONById(fileId);
    if(!text)throw new Error('File not found or no access');
    const data=JSON.parse(text);
    if(!data.years&&!data.people)throw new Error('File does not look like a Ledger data file');
    // Store the shared file ID
    DB.gdrive=DB.gdrive||{};
    DB.gdrive.sharedFileId=fileId;
    save();
    // Load the shared data
    _suppressSync=true;
    Object.assign(DB,data);
    DB.gdrive.sharedFileId=fileId; // preserve after overwrite
    try{localStorage.setItem(LS_KEY,JSON.stringify(DB));}catch(e){}
    _suppressSync=false;
    render();
    toast('Connected! You\'re now reading and writing the shared Ledger.');
  }catch(e){
    toast('Could not connect: '+e.message);
  }
}
function sharingDisconnect(){
  confirmDel('Disconnect from the shared file? You\'ll revert to your own data file.',()=>{
    DB.gdrive=DB.gdrive||{};
    delete DB.gdrive.sharedFileId;
    save();render();
    toast('Disconnected — now using your own data file');
  });
}
...
function initFYSelect(){
  ...
  sel.innerHTML=`<option value="all" ${DB.currentFY==='all'?'selected':''}>All time</option>`+
    Object.values(DB.years).filter(y=>!y.archived||String(y.startYear)===DB.currentFY).sort((a,b)=>fyOrderYear(b)-fyOrderYear(a))
    .map(y=>`<option value="${y.startYear}" ${String(y.startYear)===DB.currentFY?'selected':''}>${fyDisplay(y)}${y.locked?' 🔒':''}${y.archived?' 📦':''}</option>`).join('');
}
...
document.addEventListener('click',()=>{if(_cselOpen){_cselOpen.classList.remove('open');_cselOpen=null;}});
...
function showApp(){
  ...
  if(banner)banner.style.display=OFFLINE_MODE?'flex':'none';
  initFYSelect();autoRefreshHolidays();autoRefreshPrices();go('dashboard');
  if(!OFFLINE_MODE&&GD.isConnected())setTimeout(initDriveSync,500);
  updateSyncBtn();
}
function offlineBannerDismiss(){
  const banner=document.getElementById('offlineBanner');
  if(banner)banner.style.display='none';
}
function enterOfflineMode(){
  OFFLINE_MODE=true;
  // Build the prompt inline inside the auth gate card - more reliable than
  // using the modal system (which renders at z-index:50, behind the gate's 9999)
  const gate=document.getElementById('authGate');
  const existing=gate.querySelector('#offlinePrompt');
  if(existing)return; // already showing
  const prompt=document.createElement('div');
  prompt.id='offlinePrompt';
  prompt.style.cssText='margin-top:14px;padding:16px;background:var(--surface2);border-radius:12px;text-align:left';
  prompt.innerHTML=`
    <p style="font-size:.86rem;color:var(--muted);margin-bottom:14px;line-height:1.55">No Google Drive connection — changes will only exist in this browser.</p>
    <div style="display:flex;flex-direction:column;gap:8px">
      <button class="btn primary" style="justify-content:center;width:100%" onclick="document.getElementById('offlinePrompt')?.remove();document.getElementById('importFile').click()">
        📂 Import a backup
      </button>
      <button class="btn" style="justify-content:center;width:100%" onclick="document.getElementById('offlinePrompt')?.remove();offlineStartFresh()">
        ✦ Start fresh
      </button>
      <button class="btn ghost" style="justify-content:center;width:100%;color:var(--muted)" onclick="document.getElementById('offlinePrompt')?.remove();OFFLINE_MODE=false">
        Cancel
      </button>
    </div>`;
  // Append inside the card (the first child of the gate)
  const card=gate.querySelector('div');
  if(card)card.appendChild(prompt);
}
function offlineStartFresh(){
  // Wipe any cached data from a previous signed-in session so offline
  // mode always starts completely clean when the user requests it.
...
function showAuthGate(errorMsg){
  ...
  // Remove any inline offline prompt left over from a previous visit
  document.getElementById('offlinePrompt')?.remove();
...
async function checkAuthAndStart(){
  ...
  // Have stored credentials but token expired — try silent (no-popup) re-auth
  if(OAUTH_CLIENT_ID&&GD.cfg.userEmail){
    try{
      await GD.silentReauth();
      // If the user chose offline mode while this was in flight, don't
      // override their choice — discard the completed auth and stay offline.
      if(OFFLINE_MODE)return;
      showApp();return;
    }catch(e){/* silent re-auth failed, fall through to show gate */}
  }
  // Don't show the gate if the user already chose offline mode
  if(OFFLINE_MODE)return;
  showAuthGate();
}
async function authSignOut(){
  ...
  // Show the full-screen loader while we sync — this is the main wait point
  const loader=document.getElementById('authLoader');
  const loaderMsg=document.getElementById('authLoaderMsg');
  const showLoader=(msg)=>{
    if(loaderMsg)loaderMsg.textContent=msg||'Syncing…';
    if(loader)loader.style.display='flex';
  };
  const hideLoader=()=>{if(loader)loader.style.display='none';};
  if(GD.isConnected()){
    showLoader('Saving to Google Drive…');
    try{await pushToDrive();}catch(e){/* non-fatal */}
  }
  // Clear the auth token and stored credentials
  GD.cfg.token='';
  GD.cfg.tokenExpiry=0;
  OFFLINE_MODE=false;
  save();
  const banner=document.getElementById('offlineBanner');
  if(banner)banner.style.display='none';
