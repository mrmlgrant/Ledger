...
function expSave(i){
  ...
  // Attach receipt file if one was provided — either from the OCR scan
  // flow (_rcptPendingFile, which the user already reviewed) or via the
  // inline file picker inside the edit form.
  const fileToAttach=_rcptPendingFile||$('#f_exp_rcpt_inline')?.files[0];
  _rcptPendingFile=null; // always clear, even if we don't use it
  if(fileToAttach){
...
async function rcptView(id,name){
  ...
  const mime=rec.type||'application/octet-stream';
  const blob=rec.blob instanceof Blob?(rec.blob.type?rec.blob:new Blob([rec.blob],{type:mime})):new Blob([rec.blob||''],{type:mime});
  const url=URL.createObjectURL(blob);
  ...
  w.document.open();
  if(isImg){
    // For images: dark background viewer with a download link that carries the filename
    ...
    // For PDFs and other files: full-page embed, title = filename so browser Save uses it
    w.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>${esc2(filename)}</title>
    <style>*{margin:0;padding:0}html,body{height:100%;overflow:hidden}</style></head>
    <body><embed src="${esc2(url)}" type="${esc2(mime)}" width="100%" height="100%"></body></html>`);
  }
  w.document.close();
  // Revoke after a generous delay — enough for the PDF to load fully
