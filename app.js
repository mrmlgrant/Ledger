...
function budTotals(y,b){
  ...
  // person is set to claim them (dedPid) — defaulting to the budget's
  // first/only person when unset.
  const dedExpsByPid={};
  exps.forEach(r=>{
    const amt=budExpDeductibleAmt(r,y);
    if(amt>0){const pid=r.dedPid||pids[0];dedExpsByPid[pid]=(dedExpsByPid[pid]||0)+amt;}
  });
...
function budgetEditor(m,y,b){
  ...
  if(combined){
    const lineRows=(list)=>list.map(r=>`<tr><td>${esc(r.name||'—')}</td><td>${pdot(person(r.pid))} ${esc(person(r.pid).name)}</td><td class="num">${fmt$(yrOf(r,y))}</td></tr>`).join('');
  ...
  const expMo=budMonthlyFlow(b.exps,y);
  ...
  const yearlyExpB=(b.exps||[]).reduce((s,r)=>s+yrOf(r,y),0);
...
function renderBudgetCalc(){
  ...
  r.pids.forEach(pid=>{
    ...
    if(x.dedExps>0){
      const dedExpSubs=b.exps.filter(q=>q.isDeductible&&(q.dedPid||r.pids[0])===pid&&budExpDeductibleAmt(q,y)>0).map(q=>({n:q.name||'Expense',v:-budExpDeductibleAmt(q,y)}));
  ...
  const expSubs=b.exps.map(q=>({n:q.name||'Expense',v:-yrOf(q,y)}));
