...
function budRows(b,list,kind,withPerson){
  const psel=(r,i)=>withPerson?`<td><select class="input" onchange="budSet('${b.id}','${kind}',${i},'pid',this.value)">${DB.people.map(p=>`<option value="${p.id}" ${r.pid===p.id?'selected':''}>${esc(p.name)}</option>`).join('')}</select></td>`:'';
  const MONTHS=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
  const schedCell=(r,i)=>{
    const per=r.per||'m';
    if(per==='y'||per==='b'){
      // Month-of-occurrence picker. For bi-annual, this is the FIRST month —
      // the second occurrence is automatically +6 months later.
      const sel=`<select class="input" style="width:110px;font-size:.78rem" onchange="budSet('${b.id}','${kind}',${i},'schedDay',this.value,1)">
        ${MONTHS.map((mo,mi)=>`<option value="${mi+1}" ${String(r.schedDay)===String(mi+1)?'selected':''}>${mo}</option>`).join('')}</select>`;
      if(per==='b'){
        const m1=num(r.schedDay)||1,m2=((m1-1+6)%12)+1;
        return `<td>${sel}<div class="muted" style="font-size:.7rem;margin-top:2px">+ ${MONTHS[m2-1]}</div></td>`;
      }
      return `<td>${sel}</td>`;
    }
    if(per==='w'){
      const DOW=['Mon','Tue','Wed','Thu','Fri','Sat','Sun'];
      return `<td><select class="input" style="width:90px;font-size:.78rem" onchange="budSet('${b.id}','${kind}',${i},'schedDay',this.value,1)">
        ${DOW.map((d,di)=>`<option value="${di+1}" ${String(r.schedDay)===String(di+1)?'selected':''}>${d}</option>`).join('')}</select></td>`;
    }
    // monthly
    return `<td><input class="input" style="width:90px;font-size:.78rem" placeholder="Day (1-31)" title="Day of month (e.g. 15)" value="${r.schedDay||''}" oninput="budSet('${b.id}','${kind}',${i},'schedDay',this.value,1)"></td>`;
  };
  return list.map((r,i)=>{
    const per=r.per||'m';
    const mult=per==='w'?52:per==='b'?2:per==='y'||per==='once'?1:12;
    const yr=num(r.amt)*mult;
    const mo=mult!==12?yr/12:num(r.amt);
    const amtInput=`<input class="input money" value="${r.amt}" oninput="budSet('${b.id}','${kind}',${i},'amt',this.value,1);renderBudgetCalc()" title="Amount per ${per==='w'?'week':per==='b'?'half-year':per==='y'?'year':'month'}">`;
    // For income rows with yearly frequency, also show monthly equivalent
    const moHint=(kind==='incomes'&&per==='y'&&num(r.amt))?`<div class="muted" style="font-size:.68rem">${fmt$0(num(r.amt)/12)}/mo</div>`:'';
    const moInput=(kind==='incomes'&&per==='m')?`<div style="display:flex;align-items:center;gap:4px">
      ${amtInput}
      <div style="display:flex;flex-direction:column;gap:1px">
        <input class="input money" style="width:82px;font-size:.72rem" value="${fmt$0(yr||0).replace('$','')}" title="Yearly total — editing this sets the monthly amount" oninput="budSetYearly('${b.id}','${kind}',${i},this.value)" placeholder="Yearly">
        <span class="muted" style="font-size:.65rem;text-align:center">yearly</span>
      </div></div>`:amtInput;
    return `<tr>
    <td><input class="input" value="${esc(r.name)}" oninput="budSet('${b.id}','${kind}',${i},'name',this.value,1)"></td>
    ${psel(r,i)}
    <td class="num">${moInput}${moHint}</td>
    <td><select class="input" onchange="budSet('${b.id}','${kind}',${i},'per',this.value);renderBudgetCalc();render()">
      <option value="w" ${per==='w'?'selected':''}>Weekly</option>
      <option value="m" ${(per||'m')==='m'?'selected':''}>Monthly</option>
      <option value="b" ${per==='b'?'selected':''}>Bi-annually</option>
      <option value="y" ${per==='y'?'selected':''}>Yearly</option></select></td>
    ${schedCell(r,i)}
    ${kind==='incomes'?`<td><label style="display:flex;align-items:center;gap:4px;font-size:.74rem;white-space:nowrap" title="Tick if this figure is a package amount that already includes super, rather than a base salary super is paid on top of"><input type="checkbox" ${r.includesSuper?'checked':''} onchange="budSet('${b.id}','${kind}',${i},'includesSuper',this.checked);renderBudgetCalc();render()"> incl. super</label></td>`:''}
    <td class="num bud-yr" data-kind="${kind}" data-i="${i}">${fmt$(yrOf(r))}${kind==='incomes'&&r.includesSuper?`<div class="muted" style="font-size:.68rem">${fmt$0(budIncomeTaxable(r,FY().rates.superSGRate||11.5))} taxable</div>`:''}</td>
    <td class="rowact"><button class="btn ghost small" onclick="if(!lockedGuard()){budGet('${b.id}').${kind}.splice(${i},1);save();render()}">✕</button></td></tr>`;
  }).join('');
}
function budSet(id,kind,i,k,v,quiet){
  if(FY().locked){if(!quiet)lockedGuard();return;}
  const b=budGet(id);if(!b)return;
  b[kind][i][k]=k==='amt'?v:(k==='name'?v:v);
  save();
}
function budSetYearly(id,kind,i,yearlyStr){
  // User typed a yearly amount — convert to per-period amt and store
  if(FY().locked){lockedGuard();return;}
  const b=budGet(id);if(!b)return;
  const r=b[kind][i];if(!r)return;
  const per=r.per||'m';
  const mult=per==='w'?52:per==='b'?2:per==='y'?1:12;
  const yearly=num(yearlyStr.replace(/[^0-9.]/g,''));
  if(!isNaN(yearly)&&yearly>=0){
    r.amt=mult>0?(yearly/mult).toFixed(2):yearly;
    save();renderBudgetCalc();render();
  }
}
...
function budgetEditor(m,y,b){
  ...
  if(combined){
    ...
    incCard=`<div class="card"><div class="chead"><h2>Gross income</h2><button class="btn small" onclick="if(!lockedGuard()){budGet('${b.id}').incomes.push({name:'',amt:0,per:'m',pid:'${pids[0]}'});save();render()}">+ Row</button></div>
      <div class="cbody tight"><table class="tbl"><thead><tr><th>Source</th>${pcol}<th class="num">Amount</th><th>Per</th><th>Day/Date</th><th>Super</th><th class="num">Yearly</th><th></th></tr></thead>
      <tbody>${budRows(b,b.incomes,'incomes',hh)}</tbody></table></div></div>`;
    dedCard=`<div class="card"><div class="chead"><h2>Pre-tax deductions</h2><button class="btn small" onclick="if(!lockedGuard()){budGet('${b.id}').deds.push({name:'',amt:0,per:'m',pid:'${pids[0]}'});save();render()}">+ Row</button></div>
      <div class="cbody tight"><table class="tbl"><thead><tr><th>Deduction</th>${pcol}<th class="num">Amount</th><th>Per</th><th>Day/Date</th><th class="num">Yearly</th><th></th></tr></thead>
      <tbody>${budRows(b,b.deds,'deds',hh)}</tbody></table></div>
      <div class="cbody" style="padding-top:0"><div class="hint">e.g. investment property, novated lease, super contributions, WFH — these reduce taxable income before tax is calculated.</div></div></div>`;
    optBlock=pids.map(pid=>{
  ...
  <div class="grid2">
    <div>
      ${incCard}
      ${dedCard}
      <div class="card"><div class="chead"><h2>Post-tax expenses ${hh?'(shared)':''}</h2><button class="btn small" onclick="if(!lockedGuard()){budGet('${b.id}').exps.push({name:'',amt:0,per:'m'});save();render()}">+ Row</button></div>
      <div class="cbody tight"><table class="tbl"><thead><tr><th>Expense</th><th class="num">Amount</th><th>Per</th><th>Day/Date</th><th class="num">Yearly</th><th></th></tr></thead>
      <tbody>${budRows(b,b.exps,'exps',false)}</tbody></table></div></div>
    </div>
    <div>
...
function renderBudgetCalc(){
  ...
  $$('.bud-yr').forEach(td=>{td.textContent=fmt$(yrOf(b[td.dataset.kind][td.dataset.i]));});
  const r=budTotals(y,b);
...
function mytaxBucket(){const b=PD();return b.myTax||(b.myTax={status:'unfiled',filedDate:'',receiptId:'',receiptName:''});}
...
PAGES.networth=m=>{
  ...
  // Derive holding rows from share/ETF assets (auto-synced)
  const holdingAssets=assetsForPerson(pids).filter(a=>a.kind==='shares'||a.kind==='managed_fund');
