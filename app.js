...
function devDetail(devId, purchasedInLabel){
  ...
  const totalClaim=schedule.reduce((s,r)=>s+r.claim,0);
  ...
  const progressBar=`<div style="background:var(--surface2);border-radius:6px;height:8px;margin-top:8px;overflow:hidden">
    ...
    <div style="font-size:.78rem;color:var(--muted);margin-top:4px">${fmt$(totalClaim)} claimed of ${fmt$(workAmt)} (${pctClaimed.toFixed(0)}%)${unclaimed>0.01?' · '+fmt$(unclaimed)+' remaining':' · fully claimed'}</div>`;
